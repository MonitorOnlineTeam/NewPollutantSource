...
export const ModelNumberIdsDatas = {
  ...
  8.4: [
    ...
    'C3F2FD73-5EA7-4A6B-8036-2A6E43FD0003',
...
export const GetModelInfoByGuid = {
  ...
  '39680c93-e03f-42cf-a21f-115255251d4e': {
    method:
      '1)查看现场设备是否正常运行；2）仪表及工控机参数是否被修改；3）工控机是否有其他软件干扰监测，超过预设值按随机数处理。',
  },
  // 超过预设值有规律波动
  '983cd7b9-55e1-47f3-b369-2df7bc0a6111': {
    method:
      '1)查看现场设备是否正常运行；2）仪表及工控机参数是否被修改；3）工控机是否有其他软件干扰监测，超过预设值使用预设公式处理。',
  },
  // 超过预设值数据恒定
  '1a606047-6f21-4357-a459-03ef7788e09a': {
    method:
      '1)查看现场设备是否正常运行；2）仪表及工控机参数是否被修改；3）工控机是否有其他软件干扰监测，超过预设值使数据恒定输出。',
  },
  // 超标时数据异常陡降
  'b52087fb-563c-4939-a11f-f86b10da63c1': {
    method:
      '1)查看现场设备是否正常运行；2）仪表及工控机参数是否被修改；3）工控机是否有其他软件干扰监测，远程操控；',
  },
  // 与其他监测数据高度一致
  ...
  'c0af25fb-220b-45c6-a3de-f6c8142de8f1': {
    method:
      '1）查看现场设备是否正常运行；2）是否在同一站房内，借用合格排放口烟气通入仪表（替换分析仪监测样气）',
  },
  // 与本设备历史数据高度一致
  ...
  'f021147d-e7c6-4c1d-9634-1d814ff9880a': {
    method:
      '1）查看现场设备是否正常运行；2）是否借用本设备其他参数原始信号（如湿度信号借用到SO₂信号,断开湿度信号，观察SO₂是否变化）',
  },
  // 气态污染物波动大幅减小
  '069ab699-428a-4f4b-8df7-915d6b4f3215': {
    method:
      '1）查看现场设备是否正常运行；2）是否现场是否有人为更改分析仪量程；3）是否修改DAS软件量程或者PLC程序；',
  },
  // 颗粒物浓度波动异常
  '5bfd23c7-03da-4f4b-a258-a9c618774ab9': {
    method:
      '1）查看现场设备是否正常运行；2）有无工况变化；3）是否人为修改颗粒物截距、斜率系数、量程',
  },
};
