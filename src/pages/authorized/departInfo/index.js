...
import {
  copyObjectArrayTreeAndRenameProperty,
  permissionButton,
} from '@/utils/utils';
...
class DepartIndex extends Component {
          ...
          <span>
            ...
            {// 控制显示隐藏区域过滤
              this.props.showGroupRegionFilter && (
                <>
                  <Tooltip title={configInfo.IsOpera ? '运维区域过滤' : '区域过滤'}>
                    <a
                      onClick={() => {
                        this.setState(
                          {
           ...
           {/* {this.props.configInfo && this.props.configInfo.IsShowProjectRegion && ( */}
            ...
            {this.props.configInfo && !this.props.configInfo.IsShowProjectRegion && this.state.updateOperationArea && (
              <>
                <Divider type="vertical" />
  ...
  componentDidMount() {
    ...
    buttonList.map(item => {
      switch (item) {
        case 'testRegion':
          this.setState({ testRegionPermis: true });
          break;
        case 'SetOperationGroup':
          this.setState({ settingOperationGroupPermis: true });
          break;
        case 'updateOperationArea':
          this.setState({ updateOperationArea: true });
          break;
  ...
  saveSort = () => {
    ...
    const data = extractKeysFromTree(departInfoTree)?.[0]
      ? extractKeysFromTree(departInfoTree)
      : [];
    this.props.dispatch({
              ...
              >
                {
                  ...
                  <div>
                    <Row style={{ background: '#fff', paddingBottom: 10, zIndex: 1 }}>
                      ...
                      <Input.Group
                        compact
                        style={{ width: 290, marginLeft: 16, display: 'inline-block' }}
                      >
                        <Input
                          style={{ width: 200 }}
                          allowClear
