  ...
  renderTreeNodes = data =>
    data.map(item => {
      if (item.children && item.children.length) {
        ...
        let tempLeafTreeDatas = [...this.state.leafTreeDatas];
        if (!tempLeafTreeDatas.includes(item.key)) {
          this.setState({
            leafTreeDatas: tempLeafTreeDatas.concat(item.key),
          });
        }
      }
            ...
            <Layout style={{ padding: '14px 0', background: '#fff' }}>
              ...
              <Content style={{ padding: '0 10px', position: 'relative' }}>
                ...
                <Card bordered={false} title="角色设置" style={{ display: this.state.rolesState }}>
                  {this.props.RolesTreeDataLoading ? (
                    <Spin
                      style={{
                        width: '100%',
                        height: 'calc(100vh/2)',
