...
export default class UserInfoEdit extends Component {
  ...
  render() {
    ...
    const title =
      this.state.selectKey === 'base'
        ? '基本信息'
        : this.state.selectKey === 'roles'
        ? '角色设置'
        : '部门设置';
    return (
        ...
        <div className="contentContainer" style={{ width: '100%', background: '#fff' }}>
          {
            <Layout style={{ padding: '14px 0', background: '#fff' }}>
              <Sider width={270} style={{ background: '#fff' }}>
                ...
                >
                  {tablist.map(item => (
                    <Item key={item.key}>{item.tab}</Item>
                  ))}
                </Menu>
              </Sider>
              <Content style={{ padding: '0 10px', position: 'relative' }}>
                ...
                <Card
                  bordered={false}
                  title="基本信息"
                  style={{ height: 'calc(100vh - 160px)', display: this.state.baseState }}
                >
                  <SdlForm
                    configId="UserInfoAdd"
                ...
                <Card
                  bordered={false}
                  title="角色设置"
                  style={{ height: 'calc(100vh - 160px)', display: this.state.rolesState }}
                >
                  {this.props.UserRolesLoading ? (
                    <Spin
                      style={{
                        width: '100%',
                        height: 'calc(100vh/2)',
                ...
                <Card
                  bordered={false}
                  title="部门设置"
                  style={{ height: 'calc(100vh - 160px)', display: this.state.departState }}
                >
                  {this.props.UserDepLoading ? (
                    <Spin
                      style={{
                        width: '100%',
                        height: 'calc(100vh/2)',
                  ...
                  <Divider orientation="right" style={{ border: '1px dashed #FFFFFF' }}>
                    <Button type="primary" onClick={this.postFormDatas} loading={btnisloading}>
                      保存
                    </Button>
                  </Divider>
                </Card>
