...
@Form.create()
class MonitorPointTypeInfo extends Component {
    constructor(props) {
        super(props);
        this.state = {
            DataWhere: [],
    ...
    handleOk = e => {
        ...
        form.validateFields((err, values) => {
            if (!err) {
                ...
                dispatch({
                    type: 'BaseSetting/AddMonitorPointTypeInfo',
                    payload: {
                        PointTypeCode: formData.PointTypeCode,
                        PointTypeName: formData.PointTypeName,
                        callback: result => {
                            if (result.Datas === 1) {
                                this.setState({
                                    visible: false,
    ...
    delRowData(ID) {
        ...
        dispatch({
            type: 'BaseSetting/DelMonitorPointTypeInfo',
            payload: {
                ID,
                callback: result => {
                    debugger
    ...
    SaveOk = e => {
        ...
        form.validateFields((err, values) => {
            if (!err) {
                values.ID = keysParams["dbo.T_Bas_MonitorPointTypeInfo.ID"];
                const formData = handleFormData(values);
                dispatch({
                    type: 'BaseSetting/UpdateMonitorPointTypeInfo',
                    payload: {
                        ID: formData.ID,
                        PointTypeCode : formData.PointTypeCode,
                        PointTypeName : formData.PointTypeName,
            ...
            <BreadcrumbWrapper title="帮助中心">
                <Card>
                    ...
                    <SdlTable
                        ...
                        //         <a onClick={() => {
                        //             const keysParams = {
                        //                 'dbo.T_Bas_MonitorPointTypeInfo.ID': row['dbo.T_Bas_MonitorPointTypeInfo.ID'],
                        //             };
                        ...
                        //                 ID: row['dbo.T_Bas_MonitorPointTypeInfo.ID'],
                        //             }, () => {
                        //                 this.setState({
                        //                     Evisible: true,
                        //                 })
                        ...
                        //             title="确认是否删除?"
                        //             onConfirm={() => {
                        //                 this.delRowData(row['dbo.T_Bas_MonitorPointTypeInfo.ID']);
                        //             }}
                        //             okText="是"
                        //             cancelText="否">
                        //             <a href="#"><DelIcon /></a>
...
export default MonitorPointTypeInfo;
