...
import DeviceExceptionTypeInfoAdd from './DeviceExceptionTypeInfoAdd';
/**
 * 异常类别
 */
...
@Form.create()
class DeviceExceptionTypeInfo extends Component {
    constructor(props) {
        super(props);
        this.state = {
            // DataWhere: [],
    ...
    handleSubmit = e => {
        ...
        form.validateFields((err, values) => {
            if (!err) {
                ...
                if (!formData.ID) {
                    dispatch({
                        type: 'BaseSetting/AddExceptionTypeInfo',
                        payload: {
                            ExpectionTypeCode: formData.ExpectionTypeCode,
                            ExpectionTypeName: formData.ExpectionTypeName,
                            ParatemersName: formData.ParatemersName,
                            ...
                            callback: result => {
                                if (result.Datas === 1) {
                                    ...
                                    message.error("添加重复,请检查异常类别名称!");
                                }
                                else if (result.Datas === 3) {
                                    message.error("添加重复,请检查异常类别编号!");
                                }
                                else {
                                    message.error("添加失败!");
                                }
                    ...
                    dispatch({
                        type: 'BaseSetting/UpdateExceptionTypeInfo',
                        payload: {
                            ID: formData.ID,
                            ExpectionTypeCode: formData.ExpectionTypeCode,
                            ExpectionTypeName: formData.ExpectionTypeName,
                            ...
                            callback: result => {
                                if (result.Datas === 1) {
                                    ...
                                    message.error("修改重复,请检查异常类别名称!");
                                }
                                else if (result.Datas === 3) {
                                    message.error("修改重复,请检查异常类别编号!");
                                }
                                else {
                                    message.error("修改失败!");
                                }
    ...
    render() {
        ...
        return (
            <BreadcrumbWrapper title="设备类别信息">
                <Card>
                    <SearchWrapper
                    ...
                    <Modal
                        ...
                        width="50%"
                    >
                        <DeviceExceptionTypeInfoAdd form={this.props.form} row={this.state.row} />
                    </Modal>
                </Card>
            </BreadcrumbWrapper>
        );
...
export default DeviceExceptionTypeInfo;
