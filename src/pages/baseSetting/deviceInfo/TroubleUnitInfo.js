...
import TroubleUnitInfoAdd from './TroubleUnitInfoAdd';
/**
 * 故障单元信息
 */
...
@Form.create()
class TroubleUnitInfo extends Component {
    constructor(props) {
        super(props);
        this.state = {
            DataWhere: [],
    ...
    handleSubmit = e => {
        ...
        form.validateFields((err, values) => {
            if (!err) {
                ...
                if (!formData.ID) {
                    dispatch({
                        type: 'BaseSetting/AddTroubleUnitInfo',
                        payload: {
                            PointTypeCode: formData.PointTypeCode,
                            TroubleUnitCode: formData.TroubleUnitCode,
                            TroubleUnitName: formData.TroubleUnitName,
                            FillInType: formData.FillInType,
                            IfHost: formData.IfHost,
                            ...
                            callback: result => {
                                if (result.Datas === 1) {
                                    ...
                                    message.error("同一监测点类别下的故障单元名称不能重复!");
                                }
                                else if (result.Datas === 3) {
                                    message.error("添加重复,请检查参数编号!");
                                }
                                else {
                                    message.error("添加失败!");
                                }
                    ...
                    dispatch({
                        type: 'BaseSetting/UpdateTroubleUnitInfo',
                        payload: {
                            ID: formData.ID,
                            PointTypeCode: formData.PointTypeCode,
                            TroubleUnitCode: formData.TroubleUnitCode,
    ...
    render() {
        ...
        return (
            <BreadcrumbWrapper title="设备参数信息">
                <Card>
                    <SearchWrapper
                    ...
                    <Modal
                        ...
                        width="50%"
                    >
                        <TroubleUnitInfoAdd form={this.props.form} row={this.state.row} />
                    </Modal>
                </Card>
            </BreadcrumbWrapper>
        );
...
export default TroubleUnitInfo;
