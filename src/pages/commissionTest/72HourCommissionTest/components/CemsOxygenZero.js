...
const Index = (props) => {
    ...
    const columns2 = () => [
        ...
        },
        {
            ...
            children: [
                {
                    title: <span style={{color:'#fff',padding:!isClears&&(form.getFieldValue('SpanValue') || form.getFieldValue('SpanValue') ==0) && 4,background:form.getFieldValue('Col2')==1? '#73d13d':'#ff4d4f'}}>{!isClears && form.getFieldValue('SpanValue')}</span>,
                    align: 'center',
                    render: (text, record, index) => {
                        const obj = {
                            props: { colSpan: 0 },
