...
const dvaPropsData = ({ loading, hourCommissionTest, commissionTest, }) => ({
    formLoading: loading.effects[`${namespace}/getTemperatureCheckingRecord`],
    delLoading: loading.effects[`${namespace}/deleteTemperatureCheckingRecord`],
...
const dvaDispatch = (dispatch) => {
    return {
        ...
        getTemperatureCheckingRecord: (payload, callback) => { //参数回填
            dispatch({
                type: `${namespace}/getTemperatureCheckingRecord`,
                payload: payload,
                callback: callback
            })
        },
        addTemperatureCheckingRecord: (payload, callback) => { //保存 暂存
            dispatch({
                type: `${namespace}/addTemperatureCheckingRecord`,
                payload: payload,
                callback: callback
            })
        },
        deleteTemperatureCheckingRecord: (payload, callback) => { //删除
            dispatch({
                type: `${namespace}/deleteTemperatureCheckingRecord`,
                payload: payload,
                callback: callback
            })
        },
...
const Index = (props) => {
    ...
    const [pollutantCode, setPollutantCode] = useState(509)
    useEffect(() => {
        if (!pointId) { return }
        initData()
    }, [pointId]);
        ...
        setTimeout(() => {
            form.validateFields().then((values) => {
                ...
                if (type == 2) {   // 提交时判断日期不能一样
                    ...
                    const date2 = form.getFieldValue(`CreateDate7`).format('YYYY-MM-DD')
                    const date3 = form.getFieldValue(`CreateDate14`).format('YYYY-MM-DD')
