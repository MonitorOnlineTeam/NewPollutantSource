 ...
 const  treeLoop =(list,level = 1)=>{
    ...
    if(list&&list[0]){
     ...
     return list.map(item=>{
        ...
        return {
          ...item,
          title:<div style={{display:'inline-block',verticalAlign:'bottom',width:level==1?'calc(100% - 26px)' : 'calc(100% - 30px)'}} className='textOverflow' title={level==1? `${item.EntName}` : `${item.PointName}${item.TestCount? ` - ${item.TestCount}`: '' }` }> { level==1? `${item.EntName}`  : `${item.PointName}${item.TestCount? ` - ${item.TestCount}`: '' }` }</div>,
          key,
