...
import {
  EnumPropellingAlarmSourceType,
  EnumDYParameterException,
  EnumDataException,
  EnumDataLogicErr,
  EnumDYStatusException,
} from '@/utils/enum';
...
class EmergencyDetailInfo extends Component {
  ...
  GoToForm = (taskID, cnName, recordType, rtnVal, key, FormMainID, type) => {
    ...
    // }
    rtnVal.push(
      <p key={key} style={{ marginBottom: 0 }}>
        <Button
    ...
    TaskLogList.map(item => {
      returnStepList.push(
        <Step
          status="finish"
          title={
            item.TaskStatusText == '现场签到' ? (
              <span>
                {item.TaskStatusText}
                <EnvironmentOutlined
                  onClick={() => this.signInDetail(item)}
                  title="签到详情"
                  style={{ color: '#1890ff', paddingLeft: 4, cursor: 'pointer' }}
                />
              </span>
            ) : (
              item.TaskStatusText
  ...
  signInDetail = row => {
    ...
    this.props.dispatch({
      type: `abnormalWorkStatistics/updateState`,
      payload: { queryPar: { ...queryPar, beginTime: beginTime, endTime: endTime } },
    });
    this.props.dispatch({
    ...
    if (item.TaskRemark === '' || item.TaskRemark === null) {
      return (
        <div style={{ marginBottom: 40 }}>
          <div style={{ marginTop: 5 }}>{item.Remark}</div>
          <div style={{ marginTop: 5 }}>{item.CreateUserName}</div>
          <div style={{ marginTop: 5 }}>{item.CreateTime}</div>
        </div>
      );
    }
  ...
  getUserIcon = data => {
    ...
    if (data) {
      for (let i = 0; i < data.length; i++) {
        ...
        if (data[i].CertificateTypeID == 240) {
          // 废气
          var gasUrl = '/废气.png';
        ...
        if (data[i].CertificateTypeID == 241) {
          // 水
          var gasUrl = '/水.png';
          if (data[i].IsExpire == false) {
            gasUrl = '/水灰.png';
        ...
        if (data[i].CertificateTypeID == 242) {
          // voc
          var gasUrl = '/voc.png';
          if (data[i].IsExpire == false) {
            gasUrl = '/voc灰.png';
  ...
  taskForwardOk = () => {
    ...
    this.props.dispatch({
      ...
      payload: {
        TaskId: isExistTask && taskInfo.Datas[0].ID,
        FromUserId: isExistTask && taskInfo.Datas[0].OperationsUserId,
  ...
  getPageContent = () => {
    ...
    const { isloading, isModel } = this.props;
    if (isloading) {
      return (
        <Spin
          style={{
    ...
    // 是否存在任务信息
    const isExistTask =
      this.props.taskInfo.IsSuccess &&
      this.props.taskInfo.Datas !== null &&
      this.props.taskInfo.Datas.length > 0;
    ...
    if (isExistTask) {
      ...
      if (
        this.props.taskInfo.Datas[0].AlarmList &&
        this.props.taskInfo.Datas[0].AlarmList.length > 0
      ) {
    ...
    return (
      <div style={{ overflowY: 'hidden' }} className={styles.ExceptionDetailDiv}>
        ...
        <Card title={<span style={{ fontWeight: '900' }}>处理说明</span>} style={{ marginTop: 8 }}>
          <DescriptionList className={styles.headerList} size="large" col="1">
            <Description>
              ...
              <div>
                {isExistTask && this.props.taskInfo.Datas[0].TaskDescription
                  ? this.props.taskInfo.Datas[0].TaskDescription
                  : '没有处理说明'}{' '}
              </div>
            </Description>
          </DescriptionList>
        ...
        {isExistTask &&
          this.renderItem(
            RecordTypeInfo,
            isExistTask ? this.props.taskInfo.Datas[0].TaskID : null,
            this.props.taskInfo.Datas[0].PollutantType,
          ) &&
          this.renderItem(
            RecordTypeInfo,
            isExistTask ? this.props.taskInfo.Datas[0].TaskID : null,
            ...
            <Card
              title={<span style={{ fontWeight: '900' }}>运维台账记录</span>}
              style={{ marginTop: 8 }}
            >
              <DescriptionList className={styles.headerList} size="large" col="1">
                <Description>
              ...
              >
                ...
                <Description term="协助人">
                  {isExistTask
                    ? this.GetHelpersPeople(this.props.taskInfo.Datas[0].TaskHelpersPeople)
                    : null}
                </Description>
              </DescriptionList>
            </Card>
         ...
         *
         */}
        {isExistTask && taskInfo.Datas[0].TaskFromText === '报警响应' && (
          <Card
            title={<span style={{ fontWeight: '900' }}>报警记录</span>}
            style={{ marginTop: 8 }}
          >
            <Table
              size="small"
        ...
        !isModel && // 模型不显示
          ...
          this.props.taskInfo.Datas[0].appList?.[0] && (
            <Card
              title={<span style={{ fontWeight: '900' }}>审批记录</span>}
              style={{ marginTop: 8 }}
            >
              <Table
                size="small"
        ...
        <Modal
          ...
          destroyOnClose
          wrapClassName={isModel ? 'fullScreenModal' : 'spreadOverModal'}
          mask={false}
          footer={null}
          onCancel={() => {
        ...
        >
          <FormItem label="转发人" style={{ width: '100%' }}>
            <UserList
              onChange={value => {
                this.setState({ forwardToUserId: value });
              }}
            />
          </FormItem>
          <FormItem label="备注" style={{ width: '100%' }}>
            <Input.TextArea
              placeholder="请输入"
              onChange={e => {
                this.setState({ forwardRemark: e.target.value });
              }}
            />
          </FormItem>
        </Modal>
        ...
        {/** 现场签到 弹框 */}
        {this.state.signTitle && (
          <EntAbnormalMapModal
            abnormalTitle={this.state.signTitle}
            onCancel={() => {
              this.setState({ signTitle: undefined });
            }}
          />
        )}
      </div>
  ...
  render() {
    const { taskInfo, forwardPermis, displayContent } = this.props;
        ...
        <Card
          title={
            <Row justify="space-between">
              ...
              {forwardPermis && (
                <Button
                  disabled={isExistTask && taskInfo.Datas[0].IsForward != '1'}
                  type="primary"
                  onClick={() => this.taskForward()}
                >
                  任务转发
                </Button>
              )}
