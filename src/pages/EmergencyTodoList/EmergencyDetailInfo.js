...
class EmergencyDetailInfo extends Component {
    ...
    TaskLogList.map(item => {
      returnStepList.push(
        <Step
          ...
          icon={<Icon type={this.showIcon(item.TaskStatusText)} />}
        />,
      );
    });
  ...
  render() {
    ...
    const ImageList = [];
    fileList.map(item => {
      ImageList.push(`/uploadplantform/${item.name}`);
    });
    // 报警列表列名
    let columns = [
      {
          ...
          extra={
            <div>
              <span style={{ marginRight: 20 }}>
                {this.getCancelOrderButton(
                  isExistTask ? this.props.taskInfo.Datas[0].CreateTime : null,
                  isExistTask ? this.props.taskInfo.Datas[0].TaskStatus : null,
                )}
              </span>
              {this.getGoBack()}
            </div>
        ...
        >
          <div style={{ height: SCREEN_HEIGHT }} className={styles.ExceptionDetailDiv}>
            <Card
              ...
              title={<span style={{ fontWeight: '900' }}>基本信息</span>}
            >
              ...
              >
                <Description term="创建人">
                  ...
                  {this.getUserIcon(this.props.taskInfo.Datas[0].PeopleCertificateInfos)}
                </Description>
                <Description term="创建时间">
                  {isExistTask ? this.props.taskInfo.Datas[0].CreateTime : null}
                </Description>
              </DescriptionList>
              {(isExistTask ? this.props.taskInfo.Datas[0].TaskType : null) ===
              EnumPatrolTaskType.PatrolTask ? null : AlarmList.length === 0 ? null : (
            ...
            <Card
              title={<span style={{ fontWeight: '900' }}>处理说明</span>}
              style={{ marginTop: 20, paddingBottom: '1.5%' }}
            >
              <DescriptionList className={styles.headerList} size="large" col="1">
                <Description>
                  <TextArea
                    rows={8}
                    style={{ width: '600px' }}
                    value={isExistTask ? this.props.taskInfo.Datas[0].TaskDescription : null}
                  />
                </Description>
              </DescriptionList>
            </Card>
            <Card
              title={<span style={{ fontWeight: '900' }}>处理记录</span>}
              style={{ marginTop: 20, paddingBottom: '1.5%' }}
            >
              <DescriptionList className={styles.headerList} size="large" col="1">
                <Description>
            ...
            <Card
              title={<span style={{ fontWeight: '900' }}>附件</span>}
              style={{ marginTop: 20, paddingBottom: '1.5%' }}
            >
            ...
            <Card
              title={<span style={{ fontWeight: '900' }}>日志表</span>}
              style={{ marginTop: 20, paddingBottom: '1.5%' }}
            >
              {
          ...
          <Form className="login-form">
            <FormItem {...formItemLayout} label="说明">
              {getFieldDecorator('reason', {
                ...
                <Input.TextArea
                  rows="3"
                  prefix={<Icon type="rollback" style={{ color: 'rgba(0,0,0,.25)' }} />}
                />,
              )}
            </FormItem>
          </Form>
