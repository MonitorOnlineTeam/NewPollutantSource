...
import commonStyle from './formCommonStyle.less';

@connect(({ task, loading }) => ({
  isloading: loading.effects['task/GetBdTestRecordZB'],
  BdRecord: task.BdRecordZB,
}))
class BdTestRecordContent extends Component {
  ...
  renderCemsMainInstrument = record => {
    ...
    rtnVal.push(
      <tr key="0">
        <td style={{ width: '20%', minWidth: 200 }} className={styles.tdTitle}>
          仪器名称
        </td>
        <td style={{ width: '20%', minWidth: 200 }} className={styles.tdTitle}>
          设备型号
  ...
  renderCemsTestInfo = (recordResult, itemName) => {
    const rtnVal = [];
    let rowNum = 0;
    if (recordResult !== null && itemName !== null) {
      let result = recordResult.filter(item => item.ItemName === itemName);
      let record =
        result !== null && result !== undefined && result.length > 0 ? result[0].TestResult : null;
      if (record !== null && record !== undefined) {
          ...
          if (rowNum === 0) {
            ...
            rtnVal.push(
              <tr key={key}>
                <td>
                  {item.EndTime
                    ? `${moment(item.TestTime).format('HH:mm:ss')}~${moment(item.EndTime).format(
                        'HH:mm:ss',
                      )}`
                    : moment(item.TestTime).format('HH:mm:ss')}
                </td>
                <td>{item.CbValue}</td>
                <td>{item.CemsTextValue}</td>
                <td rowSpan={record.length + 1} style={{ textAlign: 'center' }}>
                  {result !== null && result.length > 0 ? result[0].WcValue : ''}
                </td>
                <td rowSpan={record.length + 1}>
                  {result !== null && result.length > 0 ? result[0].EvaluateStadard : ''}
                </td>
                <td rowSpan={record.length + 1} style={{ textAlign: 'center' }}>
                  {evaluateResult}
                </td>
  ...
  renderFormulaInfo = (recordResult, itemName) => {
    ...
    switch (itemName) {
      case '颗粒物':
        rtnVal.push(
          <td key="0" rowSpan="2" style={{ width: '10%' }}>
            <Checkbox checked={sltValue2}>相对误差</Checkbox>
            <br />
      ...
      case 'SO2':
        rtnVal.push(
          <td key="1" rowSpan="2" style={{ width: '10%' }}>
            <Checkbox checked={sltValue1}>相对准确度</Checkbox>
            <br />
            <Checkbox checked={sltValue2}>相对误差</Checkbox>
            <br />
            <Checkbox checked={sltValue3}>绝对误差</Checkbox>
      ...
      case 'O2':
        rtnVal.push(
          <td key="3" rowSpan="2" style={{ width: '10%' }}>
            <Checkbox checked={sltValue1}>相对准确度</Checkbox>
            <br />
            <Checkbox checked={sltValue3}>绝对误差</Checkbox>
          </td>,
      ...
      case '流速':
        rtnVal.push(
          <td key="4" rowSpan="2" style={{ width: '10%' }}>
            <Checkbox checked={sltValue2}>相对误差</Checkbox>
            <br />
            <Checkbox checked={sltValue3}>绝对误差</Checkbox>
          </td>,
      ...
      case '湿度':
        rtnVal.push(
          <td key="5" rowSpan="2" style={{ width: '10%' }}>
            <Checkbox checked={sltValue2}>相对误差</Checkbox>
            <br />
            <Checkbox checked={sltValue3}>绝对误差</Checkbox>
          </td>,
    ...
    switch (itemName) {
      case 'SO2':
        rtnVal.push(
          <tr key="0">
            <td>
              <Checkbox checked={sltValue1}>μmol/mol</Checkbox>
  ...
  renderGasInfo = record => {
    ...
    rtnVal.push(
      <tr key="0">
        <td style={{ width: '25%' }} className={styles.tdTitle}>
          标准气体名称
        </td>
        <td style={{ width: '25%' }} className={styles.tdTitle}>
          浓度值
  ...
  handleImageData = imageData => {
    ...
    return imageData?.ImgList?.map((item, index) => {
      return {
        uid: imageData.AttachID,
        name: imageData.ImgNameList[index],
        status: 'done',
        url: `/${item}`,
      };
    });
  ...
  render() {
    ...
    const StandardGasList =
      this.props.BdRecord !== null ? this.props.BdRecord.StandardGasList : null;
    const TestEquipmentList =
      this.props.BdRecord !== null ? this.props.BdRecord.TestEquipmentList : null;
    const ParamsList = this.props.BdRecord !== null ? this.props.BdRecord.ParamsList : null;
    const EquipmentInfoList =
      this.props.BdRecord !== null ? this.props.BdRecord.EquipmentInfoList : null;

    // 对比工作 - 数据对比结果
    const BDJGPic =
      this.props.BdRecord !== null ? this.props.BdRecord?.Record?.Content?.BDJGPic : {};
    // 对比工作 - 人员操作对比过程
    const BDRYPic =
      this.props.BdRecord !== null ? this.props.BdRecord?.Record?.Content?.BDRYPic : {};
    // 对比工作 - 人工设备对比过程
    const BDSBPic =
      this.props.BdRecord !== null ? this.props.BdRecord?.Record?.Content?.BDSBPic : {};
    console.log('BDJGPic', BDJGPic);
    let SignContent =
      Record !== null ? (Record.SignContent === null ? null : `${Record.SignContent}`) : null;
    ...
    return (
      <div className={styles.FormDiv} style={style}>
        ...
        <table className={styles.FormTable}>
          <tbody>
            <tr>
              ...
              <td colSpan="2">
                {Content !== null
                  ? Content.WorkingDateBegin + ' ~ ' + Content.WorkingDateEnd
                  : null}
              </td>
              <td>上次校验日期</td>
            ...
            <tr>
              <td>本次校验开始时间</td>
              <td colSpan="2">{Content !== null ? Content.CheckBTime : null}</td>
              <td>本次校验结束时间</td>
              <td colSpan="2">{Content !== null ? Content.CheckETime : null}</td>
            </tr>
          </tbody>
        </table>
        <table className={styles.FormTable}>
          <tbody>
            ...
            <tr>
              <td
                colSpan="6"
                style={{ textAlign: 'center', fontWeight: 'bold', borderBottom: '0' }}
              >
                CEMS主要仪器型号
              </td>
            </tr>
            <tr>
              <td colSpan="6" style={{ padding: '0', border: '0' }}>
                ...
                >
                  <tbody>{this.renderCemsMainInstrument(EquipmentInfoList)}</tbody>
                </table>
              </td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td colSpan="2">
                {Content !== null
                  ? Content.CurrentCheckTime
                    ? moment(Content.CurrentCheckTime).format('YYYY-MM-DD')
                    : null
                  : null}
              </td>
              <td>上次校验日期</td>
              <td colSpan="2">
                {Content !== null
                  ? Content.LastCheckTime
                    ? moment(Content.LastCheckTime).format('YYYY-MM-DD')
                    : null
                  : null}
              </td>
            </tr>
            ...
            <tr>
              ...
              <td colSpan="5" className={styles.tdTitle}>
                如校验合格前对系统进行过处理、调整、参数修改，请说明：
              </td>
            </tr>
            <tr>
              <td colSpan="5">{Content !== null ? Content.CheckConclusionResult1 : null}</td>
            </tr>
            <tr>
              <td colSpan="5" className={styles.tdTitle}>
            ...
            <tr>
              <td colSpan="5">{Content !== null ? Content.CheckConclusionResult2 : null}</td>
            </tr>
            <tr>
              <td colSpan="5" className={styles.tdTitle}>
                总体校验是否合格：
            ...
            <tr>
              <td colSpan="5">
                {Content !== null && Content.CheckIsOk !== null
                  ? Content.CheckIsOk === '1'
                    ? '合格'
                    : '不合格'
                  : null}
            ...
            <tr>
              <td
                colSpan="6"
                style={{ borderBottom: '0', textAlign: 'center', fontWeight: 'bold' }}
              >
                标准气体
              </td>
            </tr>
            <tr>
              <td colSpan="6" style={{ padding: '0', border: '0' }}>
                <table
                  ...
                  className={styles.FormTable}
                >
                  <tbody>{this.renderGasInfo(StandardGasList)}</tbody>
                </table>
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td colSpan="6" style={{ padding: '0', border: '0' }}>
                <table
                  ...
                  className={styles.FormTable}
                >
                  <tbody>{this.renderCbInfo(TestEquipmentList)}</tbody>
                </table>
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td colSpan="6" style={{ padding: '0', border: '0' }}>
                <table
                  ...
                  className={styles.FormTable}
                >
                  <tbody>{this.renderGSInfo(ParamsList)}</tbody>
                </table>
              </td>
            </tr>
            {/* <tr>
              ...
              <td colSpan="2" style={{ borderTop: '0' }}>
                {Content !== null
                  ? Content.CheckDate
                    ? moment(Content.CheckDate).format('YYYY-MM-DD')
                    : null
                  : null}
              </td>
            </tr> */}
        ...
        <div className={styles.otherContent}>
          <div className={commonStyle.uploadSection}>
            <div className={commonStyle.uploadTitle}>对比工作 - 人员操作对比过程</div>
            <SdlUpload fileList={this.handleImageData(BDRYPic)} accept="image/*" isView={true} />
          </div>
          <div className={commonStyle.uploadSection}>
            <div className={commonStyle.uploadTitle}>对比工作 - 人工设备对比过程</div>
            <SdlUpload fileList={this.handleImageData(BDSBPic)} accept="image/*" isView={true} />
          </div>
          <div className={commonStyle.uploadSection}>
            <div className={commonStyle.uploadTitle}>
              对比工作 - 数据对比结果（设备界面、数据凭条、小票等）
            </div>
            <SdlUpload fileList={this.handleImageData(BDJGPic)} accept="image/*" isView={true} />
          </div>
          <div className={commonStyle.signatureSection}>
            <div className={commonStyle.signatureTitle}>负责人签名：</div>
            <img src={`${SignContent}`} alt="签名" className={commonStyle.signatureImage} />
          </div>
        </div>
        {/* <table className={styles.FormTable}>
                    <tbody>
