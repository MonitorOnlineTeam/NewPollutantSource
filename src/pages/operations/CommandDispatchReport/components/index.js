...
class Dispatchreport extends Component {
            ...
            <div style={{ float: 'left', marginLeft: '5px', width: '95%' }}>
              ...
              >
                <div style={{ width: '95%', float: 'left', fontWeight: 'bolder' }}>
                  ...
                  <span style={{ fontSize: '10px', color: '#8c8c8c', marginLeft: '10px' }}>
                    {item.CreateTime}
                  </span>
                </div>
              ...
              <div style={{ width: '100%', marginBottom: '5px', overflow: 'hidden' }}>
                <div style={{ width: '100%', fontWeight: 'bold', fontSize: '12px' }}>
                  {item.Remark}
                </div>
              </div>
              <Steps size="small" status="process">
                {this.TaskLogList(item.Steplist)}
              </Steps>
            </div>
          </Card.Grid>
        </>,
  ...
  CommandDispatchType = (type, item) => {
    if (type == 'style') {
      switch (item) {
        case '1':
          return Style.imgstyle1;
        case '2':
          return Style.imgstyle2;
        case '3':
          return Style.imgstyle3;
        case '4':
          return Style.imgstyle4;
        default:
          return '';
      }
    } else {
    ...
    dispatch({
      ...
      payload: {
        queryparams: {
          ...
          DGIMN: dgimn,
          BTime:
            rangeDate.length > 0 && rangeDate[0] != undefined
              ? rangeDate[0].format('YYYY-MM-DD HH:mm:ss')
              : '',
          ETime:
            rangeDate.length > 0 && rangeDate[1] != undefined
  ...
  Detail = (TaskId, Type, btime, etime, EID, PID) => {
    ...
    if (Type == 1) {
      this.props.dispatch(
        routerRedux.push(`/operations/CommandDispatchReport/details/${TaskId}/${DGIMN}`),
      );
    }
    if (Type == 3) {
      this.setState({
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        align: 'left',
        render: (text, record) => (
          <Steps status="process">{this.TaskLogList(record.Steplist)}</Steps>
        ),
      },
      {
        title: '操作',
        ...
        >
          <div className={Style.Content}>
            ...
            {!dataloading &&
              (datatable.length ? (
                <>
                  <div className={Style.Item}>{this.StepsList(datatable)}</div>
          ...
          >
            <ExceptionAlarm
              ...
              Types="1"
              firsttime={moment(this.state.btime)}
