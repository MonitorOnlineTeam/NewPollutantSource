...
class TaskRecord extends Component {
    ...
    dispatch({
      ...
      payload: {
        gettasklistqueryparams: {
          ...gettasklistqueryparams,
          DGIMN:
            baseReportSearchForm.DGIMN != undefined ? baseReportSearchForm.DGIMN.join(',') : '',
          TaskCode: baseReportSearchForm.TaskCode,
          ExceptionType:
            baseReportSearchForm.ExceptionType != undefined
              ? baseReportSearchForm.ExceptionType
              : '',
          TaskFrom: baseReportSearchForm.TaskFrom != undefined ? baseReportSearchForm.TaskFrom : '',
          TaskStatus:
            baseReportSearchForm.TaskStatus != undefined ? baseReportSearchForm.TaskStatus : '',
          OperationsUserId:
            baseReportSearchForm.OperationsUserId != undefined
              ? baseReportSearchForm.OperationsUserId
              : '',
  ...
  /** 加载列表 */
  LoadData = () => {
    const { dispatch, gettasklistqueryparams } = this.props;
    dispatch({
      type: 'task/updateState',
      payload: {
        gettasklistqueryparams,
  ...
  onShowSizeChange = (pageIndex, pageSize) => {
    const { dispatch, gettasklistqueryparams, form } = this.props;
    const baseReportSearchForm = form.getFieldsValue();
    dispatch({
      type: 'task/updateState',
      payload: {
              ...
              <Tooltip title="详情">
                <a>
                  <ProfileOutlined
                    onClick={() =>
                      this.props.dispatch(
                        routerRedux.push(`/operations/taskRecord/details/${TaskID}/${DGIMN}`),
                      )
                    }
                  />
                </a>
              ...
              <Col md={8} sm={24}>
                <FormItem {...formLayout} label="监测点" style={{ width: '100%' }}>
                  {getFieldDecorator('DGIMN', {
                    initialValue: gettasklistqueryparams.DGIMN
                      ? gettasklistqueryparams.DGIMN.split(',')
                      : undefined,
              ...
              <Col md={8} sm={24} style={{ display: this.state.expand ? 'block' : 'none' }}>
                <FormItem {...formLayout} label="运维状态" style={{ width: '100%' }}>
                  {getFieldDecorator('ExceptionType', {
                    initialValue: gettasklistqueryparams.ExceptionType
                      ? gettasklistqueryparams.ExceptionType
                      : undefined,
                  })(
                    <Select
                      placeholder="请选择"
              ...
              <Col md={8} sm={24} style={{ display: this.state.expand ? 'block' : 'none' }}>
                <FormItem {...formLayout} label="任务来源" style={{ width: '100%' }}>
                  {getFieldDecorator('TaskFrom', {
                    initialValue: gettasklistqueryparams.TaskFrom
                      ? gettasklistqueryparams.TaskFrom
                      : undefined,
                  })(
                    <Select
                      style={{ width: '100%' }}
              ...
              <Col md={8} sm={24} style={{ display: this.state.expand ? 'block' : 'none' }}>
                <FormItem {...formLayout} label="任务状态" style={{ width: '100%' }}>
                  {getFieldDecorator('TaskStatus', {
                    initialValue: gettasklistqueryparams.TaskStatus
                      ? gettasklistqueryparams.TaskStatus
                      : undefined,
                  })(
                    <Select
                      placeholder="请选择"
              ...
              <Col md={8} sm={24} style={{ display: this.state.expand ? 'block' : 'none' }}>
                <FormItem {...formLayout} label="运维人" style={{ width: '100%' }}>
                  {getFieldDecorator('OperationsUserId', {
                    initialValue: gettasklistqueryparams.OperationsUserId
                      ? gettasklistqueryparams.OperationsUserId
                      : undefined,
                  })(
                    <SearchSelect
                      style={{ width: '100%' }}
                ...
                <FormItem {...formLayout} label="完成时间" style={{ width: '100%' }}>
                  {getFieldDecorator('CompleteTime', {
                    ...
                    <RangePicker_
                      ...
                      callback={(dates, type) => this.dateCallBack(dates, type, 'CompleteTime')}
                      allowClear
              ...
              <Col md={8} sm={24} style={{ display: this.state.expand ? 'block' : 'none' }}>
                <FormItem {...formLayout} label="任务类型" style={{ width: '100%' }}>
                  {getFieldDecorator('TaskType', {
                    initialValue: gettasklistqueryparams.TaskType
                      ? gettasklistqueryparams.TaskType
                      : undefined,
                  })(
                    <SearchSelect
                      style={{ width: '100%' }}
          ...
          <Form className="searchForm">
            ...
            <Row>
              ...
              >
                {getFieldDecorator('taskparent', {
                  ...
                  <Select
                    placeholder={`请选择${this.state.ParentType}`}
                    onChange={this.taskParentChange}
                  >
            ...
            <Row>
              ...
              >
                ...
                })(<Select placeholder="请选择任务类型">{this.getTaskTypeInfo()}</Select>)}
              </FormItem>
            </Row>
            <Row>
              <FormItem {...formLayout} label="描述" style={{ width: '100%', marginBottom: 10 }}>
                {getFieldDecorator('remark', {})(<TextArea placeholder="请填写描述" rows={4} />)}
              </FormItem>
            </Row>
          </Form>
        </Modal>
