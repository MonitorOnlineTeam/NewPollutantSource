 ...
 * @Last Modified time: 2025-02-13 10:11:37
 * @desc: 运维日历页面
 */
...
class CalendarPage extends PureComponent {
  ...
  componentDidMount() {
    ...
    this.props.dispatch({
      ...
      payload: { entAbnormalNumVisible: false },
    });
  }
          ...
          title: (
            <div>
              ...
              {item.ExceptionTypeText &&
                item.ExceptionTypeText.split(',').map(itm => {
                  // 报警响应异常,打卡异常,工作超时
                  let color =
                    itm === '报警响应异常' ? '#f50' : itm === '打卡异常' ? '#108ee9' : '#2db7f5';
                  return (
                    <div style={{ position: 'relative', display: 'inline-block' }}>
                      <Tag color={color}>{itm}</Tag>
                      <Popover
                        content={
                          <Button type="link" onClick={() => this.exceptionDetail(item)}>
                            详情
                          </Button>
                        }
        ...
        return {
          ...
          title: (
            <div>
              ...
              {item.ConsumablesCount ? (
                <Tag
                  color="#2db7f5"
                  style={{ cursor: 'pointer' }}
                  onClick={e => {
                    this.onTagClick(e, item, 2);
                  }}
                >
                  需更换易耗品
                </Tag>
              ...
              {item.StandardGasCount ? (
                <Tag
                  color="#108ee9"
                  style={{ cursor: 'pointer' }}
                  onClick={e => {
                    this.onTagClick(e, item, 3);
                  }}
                >
                  需更换标气
                </Tag>
              ...
              {item.V233 ? (
                <Tag color="#FACD27" style={{ cursor: 'pointer' }}>
                  需日常巡检
                </Tag>
              ) : null}
              {/* {item.V233}个 */}
          ...
          description: <div style={{ color: '#333' }}>{item.PointName}</div>,
          content: (
            <div>
  ...
  exceptionDetail = row => {
    ...
    setTimeout(() => {
      ...
      this.props.dispatch({
        type: `abnormalWorkStatistics/updateState`,
        payload: { queryPar: { ...this.props.queryPar, beginTime: beginTime, endTime: endTime } },
      });
      this.props.dispatch({
    ...
    this.props.calendarList.map(item => {
      ...
      if (item.ExcetionDate === value) {
        ...
        if (item.ExceptionTaskNum) {
          listData.push({
            color: '#f50',
            content: `异常任务${item.ExceptionTaskNum}个`,
            type: 4,
            date: item.ExcetionDate,
            text: '异常任务',
          });
        }
        // 完成任务
        if (item.CompleteTaskNum) {
          listData.push({
            color: '#87d068',
            content: `完成任务${item.CompleteTaskNum}个`,
            type: 3,
            date: item.ExcetionDate,
            text: '完成任务',
          });
        ...
        if (item.CompleteOverTaskNum) {
          listData.push({
            color: '#fa8c16',
            content: `超时完成任务${item.CompleteOverTaskNum}个`,
            type: 5,
            date: item.ExcetionDate,
            text: '超时完成任务',
          });
        ...
        if (item.ExecutedTaskNum) {
          listData.push({
            color: '#108ee9',
            content: `待执行任务${item.ExecutedTaskNum}个`,
            type: 1,
            date: item.ExcetionDate,
            text: '待执行任务',
          });
        ...
        if (item.HaveInHandTaskNum) {
          listData.push({
            color: '#2db7f5',
            content: `进行中任务${item.HaveInHandTaskNum}个`,
            type: 2,
            date: item.ExcetionDate,
            text: '进行中任务',
          });
        ...
        if (item.InCompleteOverTaskNum) {
          listData.push({
            color: '#f5222d',
            content: `超时未完成任务${item.InCompleteOverTaskNum}个`,
            type: 6,
            date: item.ExcetionDate,
            text: '超时未完成任务',
          });
        ...
        if (item.ColseTaskNum) {
          listData.push({
            color: '#bfbfbf',
            content: `系统关闭任务${item.ColseTaskNum}个`,
            type: 10,
            date: item.ExcetionDate,
            text: '系统关闭任务',
          });
        ...
        // 无异常
        if (
          !item.ExceptionTaskNum &&
          !item.CompleteTaskNum &&
          !item.ExecutedTaskNum &&
          !item.HaveInHandTaskNum &&
          !item.ColseTaskNum
        ) {
      ...
      if (item.FutureDate === value) {
        ...
        if (item.SparePartTotal) {
          listData.push({
            color: '#f50',
            content: `需备件更换${item.SparePartTotal}个`,
            type: 1,
            date: item.FutureDate,
            text: '需备件更换个数',
            future: true,
          });
        }
        ...
        if (item.ConsumablesTotal) {
          listData.push({
            color: '#2db7f5',
            content: `需易耗品更换${item.ConsumablesTotal}个`,
            type: 2,
            date: item.FutureDate,
            text: '需易耗品更换个数',
            future: true,
        ...
        if (item.StandardGasTotal) {
          listData.push({
            color: '#108ee9',
            content: `需标气更换${item.StandardGasTotal}个`,
            type: 3,
            date: item.FutureDate,
            text: '需标气更换个数',
            future: true,
        ...
        if (item.MaintainTotal) {
          listData.push({
            color: '#87d068',
            content: `需保养${item.MaintainTotal}个`,
            type: 4,
            date: item.FutureDate,
            text: '需保养个数',
            future: true,
        ...
        if (item.M233Total) {
          listData.push({
            color: '#FACD27',
            content: `需日常巡检${item.M233Total}个`,
            type: 5,
            date: item.FutureDate,
            text: '需日常巡检监测点个数',
            future: true,
    ...
    return (
      <ul className="events" style={{ width: '100%', height: '96%', overflow: 'auto' }}>
        {listData.map(item => {
          ...
          // }
          return (
            <li key={item.content} style={{ marginBottom: 2 }}>
              <Tag
                color={item.color}
                style={{ cursor: 'pointer', lineHeight: '19px', height: 20 }}
                onClick={e => {
                  e.stopPropagation();
  ...
  render() {
    ...
    const {
      ...
      columns,
      modalTableCurrent,
      taskRecordDetailVisible,
      abnormalTitle,
    } = this.state;
    const cardTitle = `${currentCellInfo.text} - ${moment(currentCellInfo.date).format(
      dateFormat,
    )}`;
    ...
    return (
      <BreadcrumbWrapper>
        <div className={styles.calendarWrapper}>
          <div style={{ display: 'flex' }}>
            <div style={{ flex: 5, marginRight: 10 }}>
              <Card className="contentContainer">
                <Form
                  // 隐藏搜索条件
                  style={{ display: this.props.location.query.hideSearch ? 'none' : '' }}
                ...
                >
                  ...
                  <Form.Item label="企业">
                    {getFieldDecorator('entCode', {
                      ...
                      <Select
                        loading={this.props.getEntByProjectInfoLoading}
                        options={this.state.entList}
                        placeholder="请选择"
                        style={{ width: 150 }}
                    ...
                    <Space>
                      ...
                      <Button
                        onClick={() => {
                          ...
                          this.setState({ projectCode: 'reset', entList: [], pointList: [] });
                        }}
                      >
                        重置
                      </Button>
          ...
          {/** 打卡异常  监测点 弹框 */}
          {this.state.abnormalTitle && (
            <EntAbnormalMapModal
              abnormalTitle={abnormalTitle}
              onCancel={() => {
                this.setState({ abnormalTitle: undefined });
              }}
            />
          )}
        </div>
