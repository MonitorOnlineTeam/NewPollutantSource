...
class DbSourceTree extends Component {
  ...
  getSearchAndDel = () => {
    return (
      <div>
        <Search
          style={{ margin: '5px 0px 5px 0px', width: '70%' }}
          placeholder="关键词"
          onChange={this.onChange}
        />
        <Button type="primary" onClick={this.delete} style={{ marginLeft: '10px' }}>
          删除
        </Button>
      </div>
    );
  ...
  getTree = () => {
    ...
    if (dbTreeArray && dbTreeArray.length > 0) {
      ...
      return (
        <img
          style={{ display: 'block', textAlign: 'center', margin: '5vh auto', width: 150 }}
          src="/nodata.png"
        />
      );
    }
