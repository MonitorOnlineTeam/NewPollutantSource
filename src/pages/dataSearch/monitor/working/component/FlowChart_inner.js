...
class FlowChart extends PureComponent {
  ...
  co2Morepar = () => {
    const { co2ju, co2x, co2l1, co2l2, co2l, co2lcj, co2jc, isCo2 } = this.state
    return <>
      {isCo2 ? <ul>
        {/* {true ? <ul> */}
        {co2ju !== '-' ? <li>截距：{co2ju} </li> : null}
        {co2x !== '-' ? <li>斜率：{co2x} </li> : null}
        {co2l1 !== '-' ? <li>量程1：{co2l1} </li> : null}
        {co2l2 !== '-' ? <li>量程2：{co2l2} </li> : null}
        {co2l !== '-' ? <li>零点校准偏差：{co2l} </li> : null}
        {co2lcj !== '-' ? <li>量程校准偏差：{co2lcj} </li> : null}
        {co2jc !== '-' ? <li>检测器信号强度：{co2jc} </li> : null}
      </ul> :
        <span>暂无数据</span>
      }
    </>
  ...
  zhiyMorepar = () => {
    ...
    return <>
      {zhiyso2 != '-' || zhiynox != '-' ? <ul>
        {zhiyso2 !== '-' ? <li>CO₂标气余量：{zhiyso2} </li> : null}
        {zhiynox !== '-' ? <li>O₂标气余量：{zhiynox} </li> : null}
      </ul>
        :
        <span>暂无数据</span>
      }
  ...
  pageContent = (type) => {
    ...
    const { yan, pi, kong, su, tan, guan, so2b, co2ND, nob, o2b, leng,
      zhikongmenState, zhikongState, tongxunState, isStop, noxb, ycnd } = this.state;

    const So2Morepar = this.so2Morepar;
    const Co2Morepar = this.co2Morepar;
    ...
    const ZHIKONG_STATUS = {
      ...
      0: <span style={{ color: green.primary, position: 'absolute', top: -64 }}>空闲</span>,
      1: <span style={{ color: blue.primary, position: 'absolute', top: -64 }} >运行</span>,
      2: <span style={{ color: gold[5], position: 'absolute', top: -64 }} >维护</span>,
      3: <span style={{ color: red.primary, position: 'absolute', top: -64 }} >故障</span>,
      5: <span style={{ color: red.primary, position: 'absolute', top: -64 }}>断电</span>,
      6: <span style={{ color: grey.primary, position: 'absolute', top: -64 }} >离线</span>,
      "-": <span style={{ color: grey.primary, position: 'absolute', top: -64 }} >离线</span>,
    };
    ...
    />  */}
      ...
      <>
        ...
        <img src="/flue.png" alt="" style={{ position: 'absolute', top: 400, left: 238, width: 120 }}></img>
      </>
      <>
      ...
      <div className={styles.ZKYMask}>

      </div>
      <>
        <div style={{ left: 728, top: 299, width: 180, height: 100, background: '#dfdfdf' }} className={`${styles.dai} ${styles.commonSty}`}>
          {ZHIKONG_STATUS[zhikongState]}
          <span style={{ margin: 6 }}>柜门：{zhikongmenState == 0 ? <span style={{ color: grey.primary }}>关</span> : <span style={{ color: red.primary }}>开</span>}</span>
      ...
      <>
        ...
        <div className={`${styles.no} ${styles.commonSty}`}>
          <span style={{ whiteSpace: 'nowrap' }}>CO₂：{co2ND}</span>
          <Tooltip placement="top" title={<Co2Morepar />} trigger='click'>
            <span className={`${styles.more}`}>更多参数</span>
          </Tooltip>
        </div>
      ...
      callback: (res) => {
        res.map((item, index) => {
          ...
          if (item.PollutantCode === "cems") {
            ...
            if (item.Code === "i33060") { //质控仪标气余量气瓶1
              this.setState({ zhiyso2: `${item.Value == null ? "-" : item.Value}${item.Unit}` })
            }
            // if (item.Code === "a05001") { //质控仪标气余量  S02
            //   this.setState({ zhiyso2: `${item.Value == null ? "-" : item.Value}${item.Unit}` })
            // }
          ...
          if (item.PollutantCode === "a05001") { //二氧化碳
            if (item.DynamicType !== "3") {
              this.setState({ isCo2: true })
            }

            if (item.DynamicType === "3") { // 标气浓度
              this.setState({ co2ND: `${item.Value == null ? "-" : item.Value}${item.Unit}` })
            }
            if (item.Code === "i13007") { //截距
              this.setState({ co2ju: `${item.Value == null ? "-" : item.Value}${item.Unit}` })
            }
            if (item.Code === "i13008") { //斜
              this.setState({ co2x: `${item.Value == null ? "-" : item.Value}` })
            }
            if (item.Code === "i13001") { //量程1
              this.setState({ co2l1: `${item.Value == null ? "-" : item.Value}${item.Unit}` })
            }
            if (item.Code === "i13050") { //量程2
              this.setState({ co2l2: `${item.Value == null ? "-" : item.Value}${item.Unit}` })
            }
            if (item.Code === "i13052") { //零点校准偏差
              this.setState({ co2l: `${item.Value == null ? "-" : item.Value}${item.Unit}` })
            }
            if (item.Code === "i13053") { //量程校准偏差
              this.setState({ co2lcj: `${item.Value == null ? "-" : item.Value}${item.Unit}` })
            }
            if (item.Code === "i13054") { //监测信号强度
              this.setState({ co2jc: `${item.Value == null ? "-" : item.Value}` })
            }
          }
