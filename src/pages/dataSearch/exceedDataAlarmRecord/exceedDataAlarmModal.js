...
class exceedDataAlarmModal extends PureComponent {
    ...
    cardTitle = () => {
        ...
        return <>
          ...
          {!regionLevel&&<>  <RegionList   style={{ width: 200, marginRight: 10 }} changeRegion={(value) => {
            ...
            <RangePicker_ format='YYYY-MM-DD'  allowClear={false} onRef={this.onRef1} isVerification={true} dateValue={dateTime} dataType={this.state.dataType} style={{ width: 400, minWidth: '200px', marginRight: '10px' }} callback={
                (dates, dataType) => {
                    this.setState({
                        time: dates
                    })
    ...
    //报警次数数据   导出
    ButtonHandleExpor=()=>{
        const {regionValue,attentionValue,outletValue,dataType,time,DealType,regionCode,enterpriseValue,PollutantCode,alarmDealTypeListCode} = this.state
        this.props.dispatch({
                    ...
                    >
                        <div style={{ marginBottom: 10 }}>
                            ...
                            {this.state.DealType === '1'?
                                ...
                                <Checkbox.Group value={this.state.alarmDealTypeListCode}  onChange={this.AlarmDealCheckBoxChange}>
                                    {
