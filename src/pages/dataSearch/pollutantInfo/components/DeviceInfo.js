...
const dvaDispatch = dispatch => {
  return {
    ...
    getPollutantById2: (payload, callback) => {
      //监测类别
      dispatch({
...
const Index = props => {
  ...
  const {
    ...
    exportLoading,
    pollutantTypeList,
    pollutantByIdLoading,
    pollutantTypeList2,
    loadingGetPollutantById2,
  } = props;
  ...
  const onFinish = async (pageIndexs, pageSizes) => {
    ...
    try {
      ...
      props.getTableData(
        ...
        () => {
          if (values.PollutantType == '31f8f6f9-5700-443b-8570-9229b36fa00c') {
            //废气
  ...
  const onValuesChange = (hangedValues, allValues) => {
    ...
    if (Object.keys(hangedValues).join() == 'Sort') {
      //设备类型
      onFinish(1, pageSize);
    }
  };
            ...
            <Spin spinning={loadingGetPollutantById2} size="small" style={{ top: -6, left: 20 }}>
              <Form.Item label="监测参数" name="PollutantCode" style={{ marginLeft: 8 }}>
                <Select
                  ...
                  style={{ width: 200 }}
                >
                  {pollutantTypeList2[0] &&
                    pollutantTypeList2.map(item => {
                      return (
                        <Option key={item.ID} value={item.ID}>
                          {item.Name}
                        </Option>
