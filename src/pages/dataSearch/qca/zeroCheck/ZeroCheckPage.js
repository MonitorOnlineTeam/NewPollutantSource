      ...
      },
      {
        title: (
          <span>
            相对误差（%）
            <QuestionTooltip
              content="在仪器未进行维修、保养或调节的前提下，CEMS 按规定的时间运行后通入零点气体，
    ...
    columns24: [
      ...
      // },
      {
        ...
        render: (text, record, index) => {
          ...
          return (
            <a style={{ color: text == 0 ? '#87d068' : '#f5222d' }}>
              {text == 0 ? '合格' : '不合格'}
            </a>
          );
        },
      ...
      <Card>
        ...
        <Tabs type="card">
          ...
          <TabPane tab="24小时零点漂移" key="2">
            <SdlTable
              loading={tableLoading}
              dataSource={zeroCheck24TableData}
              columns={columns24}
            />
          </TabPane>
