...
class Index extends Component {
  ...
  componentDidMount() {
    const { configId, UserID } = this.props;
    const DataWhere = [
      {
  ...
  /** 保存核实单 */
  handleOk = e => {
    const { dispatch, form, UserID, configId } = this.props;
              ...
              <Fragment>
                <Tooltip title="编辑">
                  <a
                    onClick={() => {
                      ...
                      const arr = row['dbo.T_Bas_CertificateInfo.AttachmentID']
                        ? row['dbo.T_Bas_CertificateInfo.AttachmentID'].split('|')
                        : [];
                ...
                <Tooltip title="删除">
                  <Popconfirm
                    title="确认要删除吗?"
                    onConfirm={() => {
                      this.delete(row['dbo.T_Bas_CertificateInfo.ID']);
                    }}
