...
class Playback extends PureComponent {
  ...
  onBackPlay = (cameraIndexCode, startTime, entTime, otherParams = {}) => {
    this.oWebControl.JS_RequestInterface({
      ...
      argument: JSON.stringify({
        cameraIndexCode: cameraIndexCode || '76ffb866c7f74b7491ea1f96ce50b97b',                   //监控点编号
  ...
  _loadJSEncryptFile = () => {
    if (window.JSEncrypt) {
      ...
      return loader('/js/video/jsencrypt.js', 'JSEncrypt')
        .then(() => {
  ...
  _loadWebControlFile = () => {
    if (window.WebControl) {
      ...
      return loader('/js/video/jsWebControl.js', 'WebControl')
        .then(() => {
          this.WebControl = window.WebControl;
          this.initPlugin();
          return Promise.resolve();
