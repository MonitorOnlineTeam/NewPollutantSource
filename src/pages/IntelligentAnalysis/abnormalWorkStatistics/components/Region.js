...
const Index = (props, ref) => {
  ...
  const [regionDetailQuery, setRegionDetailQuery] = useState({})
  ...
  const alarmResponse = () => {
    return <ol type='1' style={{ listStyleType: 'decimal' }}>
      ...
      <li>响应超时：报警首次出现后：运维工程师超过5个小时响应，则生成的工单判定为响应超时异常工单。</li>
    </ol>
  }
    ...
    },
    {
      ...
      render: (text, record, index) => {
        return <Button type="link"
          onClick={() => {
            // if (!isClockAbnormalModal) {
              setRegionDetailVisible(true)
              setRegionDetailQuery({ data: JSON.stringify(queryPar), regionName: record.regionName, regionCode: record.regionCode, abnormalTypes: abnormalTypes })
  ...
  return (
    <div style={{ height: "100%" }}>
      ...
      <Modal
        title={`
         ${regionDetailQuery.regionName} - 统计${queryPar && moment(queryPar.beginTime).format('YYYY-MM-DD')} ~ ${queryPar && moment(queryPar.endTime).format('YYYY-MM-DD')}
          内${abnormalTypes == 1 ? '打卡异常工单情况' : '报警响应超时工单情况'}
         `}
        visible={regionDetailVisible}
        onCancel={() => { setRegionDetailVisible(false) }}
        footer={null}
        destroyOnClose
        wrapClassName='spreadOverModal'
        mask={false}
      >
        <RegionDetail  location={{ query: regionDetailQuery }} />
      </Modal>
      {/*打卡异常 响应超时 弹框*/}
      <Modal
