...
import { Upload, Form, Card, Button, Select, Row, Col, Modal, DatePicker, Spin } from 'antd';
...
const dvaDispatch = dispatch => {
  return {
    ...
    getComparisonOfMonData: (payload, callback) => {
      //列表
      dispatch({
    ...
    exportTemplet: (payload, callback) => {
      // 导入模板
      dispatch({
...
const Index = props => {
  ...
  const onFinish = async (pageIndexs, pageSize) => {
    //查询
    try {
    ...
    return (
      <Card title={searchComponents()}>
        <SdlTable
          title={() => {
            return (
              <div style={{ fontSize: 22, textAlign: 'center', fontWeight: 'bold' }}>
                直测与核算碳排放量比对分析统计表
              </div>
            );
          }}
