...
import { Table, Input, InputNumber, Popconfirm, Form, Typography, Card, Button, Select, Progress, message, Row, Col, Tooltip, Divider, Modal, DatePicker, Radio, Space } from 'antd';
...
const Index = (props) => {
  ...
  const isOperaUnit = dataType==2;
  useEffect(() => {
    onFinish();

  }, [dataType]);

  const homePageIndexVal = (val)=>(val==14? '3' : val==15? '4' :  val==5? undefined : val || '1') //5是计划外 计划外不需要这个传这个参数


  const [homePageIndex, setHomePageIndex] = useState(homePageIndexVal(planType));
  const exports = async () => {
  ...
  const parentCallback = (key,outOrInsideType) => {
    // pchildref.current._childFn(values); 
    setHomePageIndex(homePageIndexVal(key))
    setOutOrInside(outOrInsideType)
  }
    ...
    >
      ...
      <Form.Item >
        <Space>
          ...
          <Button onClick={() => { form.resetFields(); onFinish(); }}>
            重置
              </Button>
    ...
    <div className={styles.planWorkOrderStatisticsDaySty}>
      <BreadcrumbWrapper hideBreadcrumb={props.hideBreadcrumb}>
        <Card title={searchComponents()} bordered={!props.hideBreadcrumb} bodyStyle={{ padding: '12px 24px' }}>
          <Region isOperaUnit={isOperaUnit} pollutantType={form.getFieldValue('pollutantType')} parentCallback={parentCallback} homePageIndexVal={homePageIndexVal} {...props} ref={pchildref} sortRate={sortRate} />
        </Card>
      </BreadcrumbWrapper>
