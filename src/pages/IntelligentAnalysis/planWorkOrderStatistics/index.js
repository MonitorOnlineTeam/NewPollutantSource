...
const dvaDispatch = dispatch => {
  return {
    ...
    regEntGetTaskWorkOrderList: payload => {
      // 计划工单统计
      dispatch({
    ...
    exportTaskWorkOrderList: payload => {
      // 导出
      dispatch({
    ...
    regEntActualGetTaskWorkOrderList: payload => {
      // 计划工单统计 实际
      dispatch({
    ...
    exportActualTaskWorkOrderList: payload => {
      // 实际 导出
      dispatch({
...
const Index = props => {
  ...
  const {
    ...
    tableLoading,
    exportLoading,
    exportActualRegLoading,
    queryPar,
    isPlanCalibrationModal,
    isPlanInspectionModal,
    isActualCalibrationModal,
    isSystemCalibrationModal,
      ...
      <Form
        ...
        initialValues={{
          pollutantType: pollutantType
            ? pollutantType
            : isPlanCalibrationModal || isPlanInspectionModal || isActualCalibrationModal || isSystemCalibrationModal
            ? props.pollutantTypes
            : 2,
          abnormalType: 1,
          time: props.time || [
            moment(new Date())
      ...
      >
        {showType == 1 ? (
          <Row align="middle">
            ...
            {(isPlanCalibrationModal || isPlanInspectionModal || isActualCalibrationModal || isSystemCalibrationModal) && (
              <Form.Item label="打卡状态" name="singinStatus" style={{ paddingRight: 8 }}>
            ...
            <Form.Item>
              ...
              <Button
                icon={<ExportOutlined />}
                loading={!isActualCalibrationModal ? exportLoading : exportActualRegLoading}
                style={{ margin: '0 8px' }}
                onClick={() => {
                  exports();
                }}
          ...
          <>
            <Row align="middle">
              <Form.Item
                ...
                name="time"
                style={{ paddingRight: 8 }}
                className={!pollutantType && 'form_label_width_83'}
              >
                <RangePicker allowClear={false} style={{ width: 240 }} format="YYYY-MM-DD" />
              </Form.Item>
            ...
            <div className={styles.planWorkOrderStatisticsSty}>
              <BreadcrumbWrapper hideBreadcrumb={props.hideBreadcrumb}>
                <Card title={searchComponents()} bodyStyle={{ padding: '0 24px' }}>
                  {showType == 1 ? (
                    <Region
                      pollutantType={form.getFieldValue('pollutantType')}
                      isPlanCalibrationModal={isPlanCalibrationModal} // 校准
                      isPlanInspectionModal={isPlanInspectionModal} // 巡检
                      isSystemCalibrationModal={isSystemCalibrationModal} // 全系统校准
                      isActualCalibrationModal={isActualCalibrationModal} 
