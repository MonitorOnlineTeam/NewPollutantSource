 ...
 */
 ...
 export default Model.extend({
   namespace: 'airQualityTrendModel',
   state: {
     exloading: false,
     loading: true,
     queryPar: {
      //MonitorTime:moment().subtract(1, 'day') .format('YYYY-MM-DD HH:00:00'),
      DataType: "HourData",
      EntCode:"d7891158-f43e-43b5-805c-ad11db586f6f",//唐银企业code
      PollutantCode:"a34001",//TSP 因子
   ...
   subscriptions: {},
   effects: {
     *getAirQualityTrend({ payload }, { call, put, update, select }) {
       //列表
       yield update({ loading:true }); 
       const response = yield call(GetAirQualityTrend, { ...payload });
       if (response.IsSuccess) {
        console.log(response.Datas)
         yield update({
           ...
           loading:false,
           pollutantTip:response.Datas.pollutantTip
         });
       }else{
