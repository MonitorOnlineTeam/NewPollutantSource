 ...
 */
 ...
 const pageUrl = {
   updateState: 'airQualityTrendModel/updateState',
   getData: 'airQualityTrendModel/getAirQualityTrend',
 };
 @connect(({ loading, airQualityTrendModel, autoForm }) => ({
   exloading: airQualityTrendModel.exloading,
   loading: airQualityTrendModel.loading,
   total: airQualityTrendModel.total,
   tableDatas: airQualityTrendModel.tableDatas,
   queryPar: airQualityTrendModel.queryPar,
   column: airQualityTrendModel.column,
   chartTime: airQualityTrendModel.timeList,
   pollutantTip:airQualityTrendModel.pollutantTip
 }))
 @Form.create()
 export default class Index extends Component {
   ...
   template = () => {
     const { dispatch, queryPar } = this.props;
     dispatch({
       type: 'airQualityTrendModel/exportSewageHistoryList',
       payload: { ...queryPar },
       callback: data => {
   ...
   getChartData = () => {
     const { chartExport, chartImport, chartTime, entName, PollutantList, column,tableDatas ,pollutantTip} = this.props;
     let pollSelect = [], pollName = []
    console.log(pollutantTip)
     if (column.length > 0) {
       column.map(item => {
         var valuedata=[]
         
         tableDatas.map(t=>{
           valuedata.push(t[item.typeCode+"_monitorValue"]);
         })
   
         pollSelect.push({ name: `${item.typeName}`, type: 'line', data: valuedata, areaStyle: {}, emphasis: {
          focus: 'series'
        } //item.data.map(items => { return items.monitorValue }) 
       })
         pollName.push(`${item.typeName}`)
       })
     }
           ...
           <>
             <Form layout="inline">
               <Row>
                 ...
                 <Form.Item >  <Select
          placeholder="请选择污染物因子"
          onChange={this.onSelectChange}
          defaultValue={"a34001"}
        >
          <Option value="a34001">TSP</Option>
          <Option value="a34002">TSP2</Option>
        </Select>
</Form.Item>
