...
const Index = props => {
  ...
  useEffect(() => {
    ...
    if (type == 1 || type == 2) {
      ...
      let data = '';
      if (type == 1) {
        data = history?.location?.query?.data
          ? JSON.parse(props?.history?.location?.query?.data)
          : '';
      } else {
            ...
            <Tooltip title="查看">
              <a
                onClick={e => {
                  ...
                  // router.push(`${detailPath}/${record.WarningCode}`);
                }}
              >
                <DetailIcon />
              </a>
  ...
  function hasConsistentProperties(array, property1, property2) {
    return array.every((obj, index, arr) => {
      ...
      return obj[property1] === prevObj[property1] && obj[property2] === prevObj[property2];
    });
  }
  ...
  const rowSelection = {
    ...
    onChange: (newSelectedRowKeys, row) => {
      if (
        ...
        if (
          currentRow?.['DGIMN'] == selectedRow['DGIMN'] &&
          currentRow?.['WarningName'] == selectedRow['WarningName']
        ) {
    ...
    <div className={styles.verificationTakeWrapper}>
      <BreadcrumbWrapper>
        ...
        >
          ...
          <Modal
            title={`${cluesDetailsProps?.EntName} / ${cluesDetailsProps?.PointName} - ${cluesDetailsProps?.WarningName}`}
          ...
          >
            {cluesDetailsProps && (
              <CluesDetails
                ...
                match={{
                  params: {
                    id: cluesDetailsProps.WarningCode,
                  },
                }}
                location={{
                  query: {
