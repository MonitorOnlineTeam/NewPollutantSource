...
const Index = props => {
  ...
  useEffect(() => {
    // 根据传入的企业获取排口列表
    if (formInitialValues.entCode) {
      getPointList(formInitialValues.entCode);
    }
  ...
  const getColumns = () => {
    const yearValue =
      queryPar && queryPar.bTime ? moment(queryPar.bTime).format('YYYY') : moment().format('YYYY');
    if (type == 1) {
      return [
        ...commonCol,
        {
        ...
        title={
          ...
          >
            <Row justify="space-between">
              ...
              <Spin spinning={!!queryLoading} size="small">
  ...
  return (
    <div className={styles.pollutantDischargeGapWrapper}>
      {showMode === 'modal' ? (
        getPageContent()
      ) : (
