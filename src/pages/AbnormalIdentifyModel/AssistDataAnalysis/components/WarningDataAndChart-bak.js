...
import { Alert, Form, Space, Button, Select, Radio, message, Spin, Badge, Row, Modal } from 'antd';
...
const WarningDataAndChart = props => {
  ...
  const [showType, setShowType] = useState(props.onlyTable ? 'data' : 'chart');
  ...
  const getOption = (legendSelected, isEditing) => {
    // if (!echartRef) {
    //   return {};
    // }
    ...
    pollutantCodes.map((pollutant, index) => {
      ...
      // let name = pollutant.PollutantName;
      ...
      yxisData.push({
        ...
        // position: position.left.includes(pollutant.PollutantCode) ? 'left' : 'right',
        alignTicks: true,
        // offset: ((idx - 1) / 2) * (spacing[name] || 44),
    ...
    let option = {
      ...
      grid: {
        top: 90,
        left: (_.values(legendSelected).filter(item => item === true).length / 2) * 40,
        right: (_.values(legendSelected).filter(item => item === true).length / 2) * 40,
        bottom: '3%',
        containLabel: true,
      },
      toolbox: {
  ...
  const onChartLegendChange = (value, param) => {
    ...
    //     // position: position.left.includes(name) ? 'left' : 'right',
    //     offset: ((idx - 1) / 2) * spacing,
    ...
    // grid.right = (yAxisSelectedLength / 2) * 30;

    // console.log('option', echarts_instance.getOption());
  ...
  const renderEcharts = useMemo(() => {
    return (
      <ReactEcharts
        ...
        theme="light"
        option={getOption(legendSelected)}
        lazyUpdate
        // notMerge
        id="rightLine"
        onEvents={onEvents}
  ...
  const editingEcharts = useMemo(() => {
    return (
      <ReactEcharts
        ...
        onChartReady={e => {}}
        theme="light"
        option={getOption(legendSelected, true)}
        // lazyUpdate
        // notMerge
        id="rightLine"
        onEvents={onEvents}
  ...
  return (
    <>
      ...
      >
        ...
        {// 弹窗不显示编辑功能
        props.displayType !== 'modal' && !onlyTable && (
          <Button
            type="primary"
