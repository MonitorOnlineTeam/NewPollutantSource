...
const Index = (props) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            ellipsis: true,
            render: (text, record) => {
                return <span style={{cursor:'pointer'}} onClick={()=>logQuery(record?.Key,1)}>{text}</span>
            }
        },
        {
            title: '处理失败',
            ...
            ellipsis: true,
            render: (text, record) => {
                return text > 0 ? <span style={{cursor:'pointer'}} onClick={()=>logQuery(record?.Key,0)} className='red' >{text}</span> : text
            }
        },
    ];
    ...
    const TitleComponents = ({ title,style }) => {
        return <div style={{ display: 'inline-block', fontSize: 18, fontWeight: 'bold', padding: '0 12px 12px 0',...style }}>{title}</div>
    }
    ...
    return (
        <div className={`${styles.characteristicLearningSty}`}>
            <BreadcrumbWrapper >
                ...
                <Row style={{ marginTop: 12, maxHeight: 'calc(100vh - 180px)', overflowY: 'auto' }}>
                        ...
                        <Spin spinning={tableLoading}>
                            <Card style={{ marginBottom: 12 }}>
                                ...
                                <Descriptions column={2}>
                                    <Descriptions.Item label="成功训练排口"><span style={{cursor:'pointer'}} onClick={()=>logQuery(trainningResult?.Key,1)}>{trainningResult?.SucNum || 0}</span></Descriptions.Item>
                                    <Descriptions.Item label="失败训练排口"><span style={{cursor:'pointer'}} onClick={()=>logQuery(trainningResult?.Key,0)}>{trainningResult?.FailNum || 0}</span></Descriptions.Item>
                                    <Descriptions.Item label="最近训练时间">{trainningResult?.LastTime || 0}</Descriptions.Item>
                                </Descriptions>
                            </Card>
                        </Spin>
                    </Col>
