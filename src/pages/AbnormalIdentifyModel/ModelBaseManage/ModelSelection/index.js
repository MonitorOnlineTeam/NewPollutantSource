...
const dvaPropsData = ({ loading, ModelBaseManage, global }) => ({
  ...
  autoMatchLoading: loading.effects[`${namespace}/AutoPointRelaModel`],
...
const Index = props => {
  ...
  const { pointListLoading, tableDatas, autoMatchLoading, tableCol, tableTotal, tableLoading } = props;
  ...
  const onValuesChange = (allValues, isPage) => {
    props.dispatch({
      ...
      payload: { ...allValues }, // projectType:1,
      callback: col => {
        let trendsCol = [];
        ...
        trendsCol = col.map(item => ({
          ...
          ellipsis: true,
          width:
            item.ModelName?.length <= 3 ? item.ModelName?.length * 30 : item.ModelName?.length * 18,
          render: (text, record) => {
            return text ? (
              <CheckCircleTwoTone style={{ fontSize: 16 }} twoToneColor="#52c41a" />
            ) : (
              <CloseCircleTwoTone style={{ fontSize: 16 }} twoToneColor="#f5222d" />
            );
          },
        }));
        // }
  ...
  const onAutoMatch = () => {
    props.dispatch({
      type: `${namespace}/AutoPointRelaModel`,
      // payload: {projectType:1, ...allValues }, // projectType:1,
  ...
  const searchComponents = () => {
    return (
      ...
      >
        ...
        <Form.Item label="关联情况" name="isRelation">
          <Select
            placeholder="请选择"
            showSearch
            allowClear
      ...
      <BreadcrumbWrapper>
        <Card title={searchComponents()}>
          <Popconfirm
            ...
            cancelText="否"
          >
            <Button type="primary" loading={autoMatchLoading} style={{ marginBottom: 10 }}>
              自动匹配
            </Button>
          </Popconfirm>
          <SdlTable
