...
const dvaDispatch = dispatch => {
  return {
    ...
    getModelWarningCheckedRectification: payload => {
      //列表
      dispatch({
    ...
    exportModelWarningCheckedRectification: payload => {
      //导出
      dispatch({
...
const Index = props => {
  ...
  let commonCol = type => [
    {
      ...
      render: (text, record) => {
        return (
          <a
            onClick={() => {
              rectificationNum(record, 0, type);
            }}
          >
            {text}
          </a>
    ...
    },
    {
      ...
      render: (text, record) => {
        return (
          <a
            onClick={() => {
              rectificationNum(record, 1, type);
            }}
          >
            {text}
          </a>
    ...
    },
    {
      ...
      render: (text, record) => {
        return (
          <a
            onClick={() => {
              rectificationNum(record, 2, type);
            }}
          >
            {text}
          </a>
        ...
        return (
          <a
            onClick={() => {
              ...
              onFinish({ ...queryPar, regionCode: regCode }, 2);
            }}
          >
            {text}
          </a>
        ...
        const element = (
          <a
            onClick={() => {
              ...
              setEntCode();
              onFinish({ ...queryPar, regionCode: cityCode }, 3);
            }}
          >
            {text}
          </a>
  ...
  const onFinish = async (queryPar, pointType) => {
    //查询
    try {
      const values = await form.validateFields();
      const par = queryPar
        ? { ...queryPar, pointType: pointType }
        : {
            ...values,
            bTime: values.time ? moment(values.time[0]).format('YYYY-MM-DD 00:00:00') : undefined,
            eTime: values.time ? moment(values.time[1]).format('YYYY-MM-DD 23:59:59') : undefined,
            time: undefined,
            pointType: pointType,
          };
  ...
  const rectificationNum = (row, status, type) => {
    ...
    onFinish(
      { ...queryPar, regionCode: regCode, dgimn: row.DGIMN && row.DGIMN, status: status },
      4,
    );
  };
  ...
  const exports = (queryPar, pointType) => {
    //导出
    ...
    return pointType == 1 ? (
      ...
      <Form layout="inline">
        <Form.Item>
          <Button
            icon={<ExportOutlined />}
            onClick={() => {
              exports({ ...queryPar, regionCode: regionCode }, 2);
            }}
      ...
      <Form layout="inline">
        <Form.Item>
          <EntAtmoList
            onChange={value => {
              onFinish({ ...queryPar, regionCode: cityCode, entCode: value }, 3);
              setEntCode(value);
            }}
            style={{ width: 260 }}
          />
        ...
        <Form.Item>
          <Button
            icon={<ExportOutlined />}
            onClick={() => {
              exports({ ...queryPar, regionCode: cityCode, entCode: entCode }, 3);
            }}
    ...
    <BreadcrumbWrapper>
      <div className={styles.equipmentManufacturListSty}>
        ...
        <RectificaRateDetail
          ...
          open={rectificaDetailVisible}
          onCancel={() => {
            setRectificaDetailVisible(false);
          }}
        />
        <Modal //整改列表
          ...
          onCancel={() => {
            setRectificationNumVisible(false);
          }}
          destroyOnClose
        >
          <SdlTable
