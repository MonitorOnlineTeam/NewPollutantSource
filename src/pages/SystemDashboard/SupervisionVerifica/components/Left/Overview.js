...
const DeviceInfoCount = props => {
  ...
  const dataList = [
    { name: '排放口数量', value: 212, iconUrl: '/SystemDashboard/SupervisionVerifica/zl_type1.png' },
    { name: '核查不规范', value: 212, iconUrl: '/SystemDashboard/SupervisionVerifica/zl_type2.png' },
    { name: '整改中', value: 212, iconUrl: '/SystemDashboard/SupervisionVerifica/zl_type3.png' },
    { name: '核查正常', value: 212, iconUrl: '/SystemDashboard/SupervisionVerifica/zl_type4.png' }
  ]
  const valSty = {
  fontSize: 20,
  fontWeight: 500,
    ...
    <div style={{ width: '100%', height: '100%', padding: '16px 0' }}>
      <Row className={`${styles.SupervisionVerificaOverviewCard}`} onClick={onOpenModal}>
        ...
        <div style={{position:'absolute',left:'calc(50% - 41px)',top:'calc(50% - 28px)',textAlign:'center'}}>
          <div><span style={{...valSty, background: 'linear-gradient(to bottom, #F6FAFC, #0FAEFF)',fontSize:32}}>562</span> 家</div>
