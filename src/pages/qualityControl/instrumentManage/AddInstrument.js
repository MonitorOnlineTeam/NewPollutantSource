...
class AddInstrument extends Component {
          ...
          render: (text, record, index) => (
            <FormItem style={{ marginBottom: '0' }}>
              {this.props.form.getFieldDecorator(`DGIMN${record.key}`, {
                rules: [{ required: true, message: '请输入排口' }],
          ...
          render: (text, record, index) => (
            <FormItem style={{ marginBottom: '0' }}>
              {this.props.form.getFieldDecorator(`MNHall${record.key}`, {
                rules: [{ required: true, message: '请输入排口通道' }],
                initialValue: text,
              })(
                // <InputNumber min={0}/>
                <Select
                  onChange={value => {
                    this.changeDataSource(value, index, 'MNHall');
                  }}
                >
                  {this._SELF_.MNHallList.map(item => (
                    <Option
                      disabled={this.state.dataSource.find(itm => itm.MNHall == item)}
                      key={item}
                    >
                      {item}
                    </Option>
                          ...
                          onOk() {
                            ...
                            let component = that.state.currentWorkPatternList.map(item => {
                              return {
                                ...
                                ...item,
                                unit:
                                  item.StandardGasCode === '03' || item.StandardGasCode === '02'
                                    ? 'mg/m3'
                                    : '%',
                              };
                            });
  ...
  onSubmitForm = () => {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      const { dataSource, dataSourceR } = this.state;
      let isErr = false;
      dataSource.map(item => {
        if (!item.Component.length) {
          this.setState({
            expandedRowKeys: [...this.state.expandedRowKeys, item.key],
          });
          isErr = true;
        }
  ...
  expandedRowRender = (record, index, indent, expanded) => {
        ...
        render: (text, record, idx) => (
          <FormItem style={{ marginBottom: '0', width: '100%' }}>
            {this.props.form.getFieldDecorator(`StandardGasCode${record.key}`, {
              rules: [{ required: true, message: '请选择标气名称' }],
              ...
              >
                {this.props.standardGasList
                  .filter(item => item.PollutantCode !== '065')
                  .map(item => (
                    <Option
          ...
          return (
            <FormItem style={{ marginBottom: '0' }}>
              {this.props.form.getFieldDecorator(`StandardValue${record.key}`, {
                rules: [{ required: true, message: '请输入配比气浓度' }],
                initialValue: text || undefined,
              })(
                <InputNumber
                  ...
                  min={0}
                  onChange={value => {
                    this.changeStandardGasData(index, 'StandardValue', value, idx);
                  }}
                />,
              )}
              &nbsp;{record.unit}
          ...
          return (
            <FormItem style={{ marginBottom: '0' }}>
              {this.props.form.getFieldDecorator(`TotalFlowSetVal${record.key}`, {
                rules: [{ required: true, message: '请输入总流量设定值' }],
                initialValue: text || undefined,
              })(
                <InputNumber
                  // formatter={value => `${value}${record.unit}`}
                  ...
                  min={0}
                  onChange={value => {
                    this.changeStandardGasData(index, 'TotalFlowSetVal', value, idx);
                  }}
                />,
              )}
            </FormItem>
          ...
          return text ? (
            <FormItem style={{ marginBottom: '0' }}>
              {this.props.form.getFieldDecorator(`Range${record.key}`, {
                rules: [{ required: true, message: '请填写满量程值' }],
                initialValue: text || undefined,
              })(
                ...
                <InputNumber
                  min={0}
                  onChange={value => {
                    this.changeStandardGasData(index, 'Range', value, idx);
                  }}
                />,
              )}
            </FormItem>
          ...
          return (
            <FormItem style={{ marginBottom: '0' }}>
              {this.props.form.getFieldDecorator(`VentilationTime${record.key}`, {
                rules: [{ required: true, message: '请选择通气时间' }],
                initialValue: text ? `${text}` : '3',
              })(
                <Select
                  onChange={value => {
                    this.changeStandardGasData(index, 'VentilationTime', value, idx);
                  }}
                >
          ...
          return (
            <FormItem style={{ marginBottom: '0' }}>
              {this.props.form.getFieldDecorator(`StabilizationTime${record.key}`, {
                rules: [{ required: true, message: '请选择稳定时间' }],
                initialValue: text ? `${text}` : '3',
              })(
                <Select
                  onChange={value => {
                    this.changeStandardGasData(index, 'StabilizationTime', value, idx);
                  }}
                >
                  {timeList.map(item => item)}
                </Select>,
    ...
    const scrollXWidth = columns
      .map(col => col.width || 150)
    ...
    const columnsR = [
          ...
          <FormItem style={{ marginBottom: '0', width: '100%' }}>
            {this.props.form.getFieldDecorator(`StandardGasCode${record.key}`, {
              ...
              >
                {this.props.standardGasList.map(item => (
                  <Option
                    disabled={this.state.dataSourceR.find(
                      itm => itm.StandardGasCode == item.PollutantCode,
                    )}
                    key={item.PollutantCode}
                    value={item.PollutantCode}
                  >
      ...
      },
      {
        ...
        render: (text, record, idx) => {
          ...
          return (
            <FormItem style={{ marginBottom: '0' }}>
              {this.props.form.getFieldDecorator(`Concentration${record.key}`, {
                rules: [{ required: true, message: '请填写气瓶标气浓度' }],
                initialValue: text || undefined,
              })(
                <InputNumber
                  min={0}
                  precision={2}
                  onChange={value => {
                    this.changeStandardGasDataR('Concentration', value, idx);
                  }}
                />,
              )}{' '}
              &nbsp;{record.unit}
      ...
      },
      {
        ...
        render: (text, record, idx) => (
          <FormItem style={{ marginBottom: '0' }}>
            {this.props.form.getFieldDecorator(`GasInitPower${record.key}`, {
              rules: [{ required: true, message: '请填写标气初始压力' }],
              initialValue: text || undefined,
            })(
              <InputNumber
                // formatter={value => `${value}mpa`}
                ...
                min={0}
                onChange={value => {
                  this.changeStandardGasDataR('GasInitPower', value, idx);
                }}
              />,
            )}
            &nbsp;mpa
        ...
        <Card>
          ...
          <Row>
            <Divider orientation="right">
              <Button type="primary" loading={this.props.btnloading} onClick={this.onSubmitForm}>
                保存
              </Button>
              <Button
                style={{ marginLeft: 8 }}
