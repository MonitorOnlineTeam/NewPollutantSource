...
class PlaybackPage extends PureComponent {
    ...
    QCPlaybackTimeLine.map((item, index) => {
      timelineItems.push(
        <Timeline.Item
          dot={
            <LegacyIcon
              type={iconTypeByQCType[item.QCType]}
              style={{ fontSize: '16px', color: QCStatusColor[item.QCType] }}
            />
          }
          color="red"
        ...
        >
          <span
            ...
            onClick={() => {
              this.onTimeLineItemClick(item, index);
            }}
          >
            {`${item.UserName}在${item.BeginTime}发起${item.PollutantName}-${item.StandValue}${item.Unit}质控`}
            <span style={{ color: item.Color }}>{item.Result}</span>
            <Tag style={{ cursor: 'pointer' }} color={QCStatusColor[item.QCType]}>
              {item.Flag}
            </Tag>
          </span>
        </Timeline.Item>,
  ...
  render() {
    ...
    const {
      ...
      start,
      count,
      QCALineItem: { ID, QCType, DGIMN, QCAMN, QCTime, PollutantCode },
    } = this.state;
    return (
      <>
              ...
              <Row gutter={10} style={{ height: 'calc(100vh - 284px)' }}>
                <Col xxl={7} xl={9} style={{ height: '100%' }}>
                  ...
                  >
                    <Spin spinning={timeLineLoading}>
                      {!timeLineLoading && !QCPlaybackTimeLine.length ? (
                        <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} />
                      ) : (
                        <Timeline style={{ margin: '22px 0 0 22px' }}>
                          {QCPlaybackTimeLine.length && (
                            <Timeline.Item
                              dot={
                                <ClockCircleOutlined
                              ...
                              style={{ paddingBottom: 50 }}
                            >
                              <Tag className={styles.dateContent}>
                                {moment(this.state.selectedTime[0]).format('YYYY-MM-DD')} -{' '}
                                {moment(this.state.selectedTime[1]).format('YYYY-MM-DD')}
                              </Tag>
                            </Timeline.Item>
                          )}
                ...
                <Col xxl={17} xl={15} style={{ height: '100%' }}>
                  <Card
                    ...
                    size="small"
                    title="质控流程图"
                    bodyStyle={{ height: 'calc(100vh - 308px)', overflow: 'hidden' }}
                  >
                    <Spin
                      spinning={!!(flowChartLoading || otherLoading)}
                      wrapperClassName={styles.spinWrapper}
                      style={{ position: 'relative', height: '100%' }}
                    >
                      {this.returnQCStatus()}
                      {PollutantCode && PollutantCode !== 'P' && (
                        <Button
                          // size="small"
                      ...
                      {this.state.imgShow ? (
                        this.loadingImg(imgChartLoading)
                      ) : (
                        <Empty style={{ marginTop: 70 }} image={Empty.PRESENTED_IMAGE_SIMPLE} />
                      )}
                      {currentTimeLineItem &&
                      currentTimeLineItem.QCType == 1 &&
                      QCAFlowChartAllData.length ? (
                        // {true ?
