...
class index extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (this.props.timeList !== nextProps.timeList) {
      ...
      if (timeData) {
        let n = moment(nextProps.startTime)
          .add(nextProps.stabilizationTime, 'minutes')
          .valueOf();
        ...
        {showType === 'chart' ? (
          <>
            <div className={styles.legendNumBox}>
              <span>
                <span style={{ color: '#56f485', paddingLeft: 10 }}>
                  {valueList[valueList.length - 1]}
                </span>
                <span style={{ color: '#c23531' }}>
                  {standardValueList[standardValueList.length - 1]}
                </span>
