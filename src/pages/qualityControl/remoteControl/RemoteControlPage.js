...
class RemoteControlPage extends Component {
  ...
  componentDidMount() {
    ...
    this.props.dispatch({
      ...
      payload: { qcamn: this.state.QCAMN },
    });
  }
  ...
  getResidueIcon = (VolumeValue, GasInitPower) => {
    ...
    if (value <= 25) {
      icon = (
        <CustomIcon
          type="icon-dianliang"
          style={{ fontSize: 20, margin: '0 10px', color: 'red' }}
        />
      );
    } else if (value > 25 && value <= 50) {
      icon = (
        <CustomIcon
          type="icon-dianliang1"
          style={{ fontSize: 20, margin: '0 10px', color: '#32c066' }}
        />
      );
    } else if (value > 50 && value <= 75) {
      icon = (
        <CustomIcon
          type="icon-dianliang2"
          style={{ fontSize: 20, margin: '0 10px', color: '#32c066' }}
        />
      );
    } else if (value > 75) {
                              ...
                              <Row style={{ width: '100%' }} className="center">
                                ...
                                <Col span={4}>
                                  <Button
                                    type="primary"
                                    onClick={() => {
                                      this.cancelPlan(item.DGIMN, item.StandardPollutantCode);
                                    }}
                                  >
                                    取消质控
                                  </Button>
                      ...
                      {getFieldDecorator('StandardPollutantCode', {
                        ...
                        <Select
                          ...
                          onChange={(value, option) => {
                            form.setFieldsValue({
                              OldStandardValue: option.props['data-concentration'],
                            });
                            this.setState({
                            ...
                            if (value == '02' || value == '03') {
                              form.setFieldsValue({
                                OldStandardUnit: 'mg/m3',
                                MatchStandardUnit: 'mg/m3',
                              });
                            } else {
                        ...
                        >
                          {standardGasList
                            .filter(itm => itm.PollutantCode !== '065')
                            .map(item => {
                              return (
                                <Option
                                  ...
                                  data-totalFlowSetVal={item.TotalFlowSetVal}
                                >
                                  {item.PollutantName}
                    ...
                    <Form.Item label="总流量设定值">
                      {getFieldDecorator('FlowValue', {
                        rules: [
                          {
                            required:
                              form.getFieldValue('StandardPollutantCode') !== 'P' ? true : false,
                  ...
                  <Col span={12}>
                    <Form.Item label="配比标气浓度设定值">
                      {getFieldDecorator('MatchStandardValue', {
                        ...
                        <InputNumber
                          disabled={form.getFieldValue('StandardPollutantCode') == 'P'}
                          placeholder="请输入配比标气浓度设定值"
                          style={{ width: '70%' }}
                          min={0}
                          max={form.getFieldValue('OldStandardValue')}
                          precision={1}
                        />,
                      )}
                    ...
                    <Form.Item label="排口">
                      {getFieldDecorator('DGIMN', {
                        ...
                        >
                          {CEMSList.map(item => {
                            return (
                              <Option key={item.DGIMN} MNHall={item.MNHall}>
                                {item.PointName}
                              </Option>
                            );
                          })}
