...
const dvaDispatch = dispatch => {
  return {
    ...
    GetExceptionSignTaskRate: payload => {
      //现场打卡异常
      dispatch({
    ...
    GetOpertionExceptionList: payload => {
      //异常设备统计
      dispatch({
...
const Index = props => {
  ...
  const getExceptionSignTaskRate = date => {
    //现场打卡异常
  ...
  const getOpertionExceptionList = date => {
    //设备异常总览
          ...
          label: {
            ...
            rich: {
              name: {
                ...
                color: '#fff',
                align: 'left',
                padding: smallResolution
                  ? [fontSizeFn(5), fontSizeFn(2), fontSizeFn(18), fontSizeFn(2)]
                  : [5, 5, 18, 5], //上左下右 逆时针
              },
              num: {
                fontSize: fontSizeFn(20),
          ...
          data:
            pollutantType == 1
              ? [
                  ...commonData,
                  {
                    value: consumablesList.standardLiquidRepalceCount,
                    name: '试剂更换数量',
                    itemStyle: {
                      normal: {
                        color: {
              ...
              : [
                  ...commonData,
                  {
                    value: consumablesList.standardGasRepalceCoun,
                    name: '标准气体更换数量',
                    itemStyle: {
                      normal: {
                        color: {
  ...
  const deviceAbnormalOption = type => {
    //设备异常统计图表
    const { opertionExceptionList } = props;
    let color1 = ['#FFCC00', '#323A70'],
      color2 = ['#FF0000', '#323A70'],
      color3 = ['#3571EA', '#323A70'];
    let option = {
      tooltip: {
        show: false,
        trigger: 'item',
      ...
      title: {
        text:
          type == 1
            ? opertionExceptionList.intactRate == '-'
              ? '-'
              : ` ${opertionExceptionList.intactRate}%`
            : type == 2
            ? opertionExceptionList.failureRate == '-'
              ? '-'
              : `${opertionExceptionList.failureRate}%`
            : opertionExceptionList.repairRate == '-'
            ? '-'
            : `${opertionExceptionList.repairRate}%`,
        left: 'center',
        top: 'center',
            ...
            },
            {
              value:
                type == 1
                  ? opertionExceptionList.intactRate == '-'
                    ? 0
                    : 100 - opertionExceptionList.intactRate
                  : type == 2
                  ? opertionExceptionList.failureRate == '-'
                    ? 0
                    : 100 - opertionExceptionList.failureRate
                  : opertionExceptionList.repairRate == '-'
                  ? 0
                  : 100 - opertionExceptionList.repairRate,
              name: '未完成',
            },
          ],
          minAngle: 0, //最小角度
  ...
  const deviceFailureRate = () => {
    //设备故障率
    if (operationSettingInfo?.Availability == 1) {
      //提取数据 评估中心
      setEquipmentAbnormalRateVisible(true);
      setDeviceType(0);
    } else {
  ...
  const deviceAbnormalEcharts = useMemo(() => {
    ...
    return (
      <Row type="flex" align="middle" justify="space-between" style={{ width: '100%' }}>
        <Col span={8} align="middle" onClick={deviceAbnormals} style={{ cursor: 'pointer' }}>
          <StatisticNumber
            value={
              opertionExceptionList.intactRate !== '-'
                ? opertionExceptionList.intactRate + '%'
        ...
        <Col span={8} align="middle" onClick={deviceFailureRate} style={{ cursor: 'pointer' }}>
          <StatisticNumber
            value={
              opertionExceptionList.failureRate !== '-'
                ? opertionExceptionList.failureRate + '%'
        ...
        <Col span={8} align="middle" onClick={deviceFailurerePairRate} style={{ cursor: 'pointer' }}>
          <StatisticNumber
            value={
              opertionExceptionList.repairRate !== '-'
                ? opertionExceptionList.repairRate + '%'
    ...
    <Row style={{ flexFlow: 'row nowrap' }} justify="space-between">
      {exceptionSignTaskRateLoading ? (
        ...
        <Spin spinning={false}>
          <Col className={styles.clockAbnormal}>
            <CardHeader
              btnClick={clockAbnormalClick}
              showBtn
              type="week"
              btnCheck={clockBtnCheck}
              title="现场打卡异常"
            />
            <div style={{ padding: '1.3125rem', height: 'calc(100% - 2.75rem)' }}>
              <Row
              ...
              >
                <Col flex="4">
                  <StatisticNumber
                    value={exceptionSignTaskRateList.insidePlanRate + '%'}
                    text={'计划内打卡异常率'}
                    uiDisplayType={1}
                  />
                </Col>
                <Col flex="6">
                  <div className={styles.itemBox}>
                    ...
                    <span className={styles.rate}>
                      {exceptionSignTaskRateList.insidePlanTaskCount}
                    </span>
                  </div>
                  <div className={styles.itemBox} style={{ marginTop: '.625rem' }}>
                    <span>计划内打卡异常次数(次)</span>
                    <span className={styles.rate}>
                      {exceptionSignTaskRateList.insidePlanTaskExceptionCount}
                    </span>
                  </div>
                </Col>
              </Row>
              <div></div>
              <Row
                style={{ display: 'flex', alignItems: 'center', marginTop: '1.25rem', cursor: 'pointer' }}
                onClick={() => {
                  setClockAbnormalVisible(true);
                }}
              >
                <Col flex="4">
                  <StatisticNumber
                    value={exceptionSignTaskRateList.outPlanTaskRate + '%'}
                    text={'计划外打卡异常率'}
                    uiDisplayType={1}
                  />
                </Col>
                <Col flex="6">
                  <div className={styles.itemBox}>
                    <span>计划外打卡次数(次)</span>
                    <span className={styles.rate}>
                      {exceptionSignTaskRateList.outPlanTaskCount}
                    </span>
                  </div>
                  <div className={styles.itemBox} style={{ marginTop: '.625rem' }}>
                    <span>计划外打卡异常次数(次)</span>
                    <span className={styles.rate}>
                      {exceptionSignTaskRateList.outPlanTaskExceptionCount}
                    </span>
                  </div>
                </Col>
              </Row>
      ...
      )}
      {
        ...
        <Spin spinning={consumablesLoading}>
          <Col
            className={styles.consumablesStatistics}
            style={{ display: 'flex', flexDirection: 'column' }}
          >
            <CardHeader
              btnClick={consumablesClick}
              showBtn
              type="week"
              btnCheck={consumablesCheck}
              title="耗材统计"
            />
            <Row
              justify="center"
              align="middle"
              className={styles.consumablesChart}
              style={{ flex: 1, padding: 0 }}
            >
              <Col span={7} style={{ paddingLeft: '.625rem' }}>
                <StatisticNumber
                  value={
                    pollutantType == 1
                      ? consumablesList.standardLiquidRepalceCount
                      : consumablesList.standardGasRepalceCoun
                  }
                  text={
                    <div style={{ lineHeight: '20px' }}>
                      <p>{pollutantType == 1 ? '试剂' : '标准气体'}</p>
                      <p>更换数量</p>
                    </div>
                  }
                  uiDisplayType={3}
              ...
              <Col span={10}>
                <StatisticNumber
                  value={consumablesList.sparePartReplaceRecordCount}
                  text={
              ...
              <Col span={7} style={{ paddingRight: '.625rem' }}>
                <StatisticNumber
                  value={consumablesList.consumablesReplaceCount}
                  text={
                    <div style={{ lineHeight: '1.25rem' }}>
                      <p>易耗品</p>
                      <p>更换数量</p>
      ...
      <Spin spinning={exceptionListLoading}>
        <Col className={styles.deviceAbnormal}>
          {/**设备异常总览 */}
          <CardHeader
            btnClick={deviceAbnormalClick}
            showBtn
            type="week"
            btnCheck={deviceAbnormalCheck}
            title="设备异常总览"
          />
          <div style={{ height: 'calc(100% - 24px)' }} className="center">
            {deviceAbnormalEcharts}
            {/**当图表有点击事件时 更新更新页面时  图表抖动 */}
          </div>
        </Col>
      </Spin>
      ...
      <ConsumablesStatisticsModal //耗材统计弹框
        ...
        onCancel={() => {
          setConsumablesStatisticsVisible(false);
        }}
      ...
      <EquipmentAbnormalRateModal //设备完好率弹框
        ...
        type={Number(pollutantType)}
        onCancel={() => {
          setEquipmentAbnormalRateVisible(false);
        }}
      ...
      <EquipmentFailureRate //设备故障率弹框
        ...
        type={Number(pollutantType)}
        onCancel={() => {
          setEquipmentFailureRateVisible(false);
        }}
        time={[moment(deviceAbnormalCheck.beginTime), moment(deviceAbnormalCheck.endTime)]}
      />
      <EquipmentFailurerePairRate //设备故障修复率弹框
        ...
        type={Number(pollutantType)}
        onCancel={() => {
          setEquipmentFailurerePairRateVisible(false);
        }}
        time={[moment(deviceAbnormalCheck.beginTime), moment(deviceAbnormalCheck.endTime)]}
      />
    </Row>
