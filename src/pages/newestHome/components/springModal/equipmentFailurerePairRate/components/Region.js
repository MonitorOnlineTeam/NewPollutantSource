...
const dvaDispatch = dispatch => {
  return {
    ...
    regGetRepairRateList: (payload, callback) => {
      // 行政区
      dispatch({
...
const Index = props => {
  ...
  const [currentRegionDetails, setCurrentRegionDetails] = useState({});
  const regionDetail = (row, par) => {
    setRegionDetailVisible(true);
    setCurrentRegionDetails(row);
  ...
  return (
    <div className={styles.equipmentFailurerePairRateSty}>
      {!regionDetailVisible ? (
        ...
        <Modal
          title={`设备故障修复率 - ${currentRegionDetails.regionName}`}
          wrapClassName="spreadOverModal"
          mask={false}
          open={regionDetailVisible}
          footer={false}
          onCancel={() => {
