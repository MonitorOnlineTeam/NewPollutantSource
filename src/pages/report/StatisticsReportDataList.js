    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, row, index) => {
          return (
            <span
              className={style.entClick}
              onClick={() =>
                dispatch(
                  routerRedux.push(
                    `/Intelligentanalysis/SewagePlant/dataReportList/statisticsReportDataList/statisticsReportDataListView/DataReportingView/${row.MonitorTime}/${row.EntCode}`,
                  ),
                )
              }
            >
