...
const Index = (props) => {
  ...
  const {location:{pathname}, queryPar, tableDatas, tableTotal, tableLoading, auditPhotoLoading, installPhotoData, addAuditInfoLoading,exportLoading} = props;
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text) => {
        if(type==1){
        return  <span style={{color: text == '审核未通过'? '#f5222d' : 'rgba(0, 0, 0, 0.85)'}}>{text}</span> 
        }else{
        return  <span style={{color:text == '优秀' ? '#52c41a' :  'rgba(0, 0, 0, 0.85)'}}>{text}</span>
    ...
    },
    {
      ...
      render: (text, record,index) => {
        ...
        return type==1? record.StatusName=='待审核'&&(
          <Tooltip  placement={disabledFlag? 'left' : 'top'} title={disabledFlag? "无设备型号，暂不支持审核": "审核"}>
            <a
             style={{ cursor: disabledFlag && 'not-allowed', color: disabledFlag &&'rgba(0, 0, 0, 0.25)' }}
              onClick={() => {
          ...
          <Tooltip title="导出">
            <a
              onClick={() => {
                ...
                exports({serviceAreaCode:record.ServiceAreaCode, num:record.Num,projectCode:record.ProjectCode,projectName:record.ProjectName})
              }}
            >
          
             {index==exportIndex&&exportLoading?  <Spin size='small'/> : <ExportOutlined style={{ fontSize: 16 }} /> }
            </a>
          </Tooltip>
        )
        ;
  ...
  const searchComponents = () => {
    ...
    >
      <Row align='middle'>
        ...
        {type==2&&<Col span={8}><Form.Item name='time' label='离开现场时间' >
              <RangePicker_ 
              format="YYYY-MM-DD"
              style={{ width: '100%' }}
              />
      ...
      <BreadcrumbWrapper>
        <Card title={searchComponents()}>
          ...
          >
           ...
           <ExamineModal   visible={examineVisible}    title={examineTitle} data={examineData}  onCancel={() => { setExamineVisible(false) }} onFinish={()=>{onFinish(pageIndex, pageSize)}}/>
        </Card>
      </BreadcrumbWrapper>
    </div>
  );
