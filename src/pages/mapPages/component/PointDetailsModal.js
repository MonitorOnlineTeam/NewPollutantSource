...
@connect(({ loading, map, global, user }) => ({
...
class PointDetailsModal extends PureComponent {
  ...
  render() {
    const { menuDescList, infoWindowData, pointInfo, pointDetailsModalVisible } = this.props;
    ...
    return (
      ...
      >
        <Tabs>
          ...
          {
            menuDescList.includes('超标处置') && this.state.currentPointInfo.type != '5' &&
          ...
          {
            menuDescList.includes('超标数据') && this.state.currentPointInfo.type != '5' &&
