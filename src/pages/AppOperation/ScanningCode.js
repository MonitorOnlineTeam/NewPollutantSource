...
class ScanningCode extends PureComponent {
  ...
  render() {
    ...
    if (
      operationLogList &&
      operationLogList.operationStatesCount &&
      operationLogList.operationStatesCount.length > 0
    ) {
            ...
            <Step
              ...
              description={
                <div style={{ fontSize: 13 }}>
                  ...
                  >
                    运维人:
                    {itemChild.User_Name.length > 3
                      ? itemChild.User_Name.substring(0, 3) + '..'
                      : itemChild.User_Name}
                  </div>
                  <div
                    style={{
      ...
      <div style={{ height: '100vh' }}>
        <Card full style={{ height: 150 }}>
          <Card.Header
            title={
              <span>
                <span style={{ fontSize: 15 }}>工单情况</span>
              ...
              >
                <div style={{ height: 25, width: 90, marginTop: 4, float: 'right' }}>
                  ...
                  <div arrow="horizontal" style={{ fontSize: 15, float: 'right' }}>
                    {moment(operationRzWhere.beginTime).format('YYYY-MM')}{' '}
                  </div>
                </div>
              </DatePicker>
            }
        ...
        >
          ...
          {this.props.loading ? (
            ...
            operationLogList.FormList && operationLogList.FormList.length > 0 ? (
              <Steps size="small">{gridList}</Steps>
            ) : (
              <div style={{ width: '100%', textAlign: 'center', paddingRight: 20 }}>暂无数据</div>
            )
          ) : (
            <div style={{ width: '100%', textAlign: 'center', paddingRight: 20 }}>暂无数据</div>
          )}
        ...
        <Modal
          ...
          footer={[
            {
              text: <span style={{ fontSize: 13 }}>知道了</span>,
              onPress: () => {
                this.onClose()();
              },
            },
        ...
        >
          <List renderHeader={() => '工单状态说明'} className="my-list">
            ...
            <Item wrap>
              <span style={{ fontSize: 13 }}>2.完成：运维人员完成了的工单；</span>
            </Item>
            <Item wrap>
              <span style={{ fontSize: 13 }}>3.关闭：运维人员已经关闭的工单；</span>{' '}
            </Item>
            <Item wrap>
              <span style={{ fontSize: 13 }}>4.执行中：运维人员正在处理的工单；</span>{' '}
            </Item>
            <Item wrap>
              <span style={{ fontSize: 13 }}>5.待执行：运维人员还未处理的工单。</span>{' '}
            </Item>
          </List>
        </Modal>
      </div>
