...
class OperationFormDetail extends PureComponent {
    ...
    render() {
        ...
        let ShowAttachment = [];
        const URLattachment = [];
        if (OperationFormDetail && OperationFormDetail[0] && OperationFormDetail[0].Attachments && OperationFormDetail[0].Attachments.ThumbimgList && OperationFormDetail[0].Attachments.ThumbimgList.length !== 0) {
            ...
            <Card full  style={{ height: '100vh', overflow: 'scroll' }}>
                <Card.Header
                    ...
                    extra={
                        <div style={{ height: 25, width: 90, marginTop: 4, float: 'right' }}>
                            ...
                            <div arrow="horizontal" style={{ fontSize: 15, float: 'right' }}><span style={{ fontSize: 15 }} onClick={() => { this.backPages() }}>返回</span> </div>
                        </div>
                    }
                />
                <Card.Body>
                    <List renderHeader={() => '基本信息'} className="my-list">
                        ...
                        <Item ><span style={{ fontSize: 13 }}> 任务类型：{OperationFormDetail && OperationFormDetail.length > 0 ? OperationFormDetail[0].TaskTypeText : null}</span></Item>
                        <Item ><span style={{ fontSize: 13 }}> 创建人：{OperationFormDetail && OperationFormDetail.length > 0 ? OperationFormDetail[0].CreateUserName : null}</span></Item>
                ...
                {this.state.previewVisible && (
                    <Lightbox
                        ...
                        nextSrc={UrlTaskDitailsAttachmentList[(photoIndex + 1) % UrlTaskDitailsAttachmentList.length]}
                        prevSrc={UrlTaskDitailsAttachmentList[(photoIndex + UrlTaskDitailsAttachmentList.length - 1) % UrlTaskDitailsAttachmentList.length]}
                        onCloseRequest={() => this.setState({ previewVisible: false })}
                        onPreMovePrevRequest={() =>
                            this.setState({
                                photoIndex: (photoIndex + UrlTaskDitailsAttachmentList.length - 1) % UrlTaskDitailsAttachmentList.length
                            })
                        }
                ...
                {this.state.previewVisibleAttach && (
                    <Lightbox
                        ...
                        prevSrc={URLattachment[(photoIndexAttach + URLattachment.length - 1) % URLattachment.length]}
                        onCloseRequest={() => this.setState({ previewVisibleAttach: false })}
                        onPreMovePrevRequest={() =>
                            this.setState({
                                photoIndexAttach: (photoIndexAttach + URLattachment.length - 1) % URLattachment.length
                            })
                        }
                        onPreMoveNextRequest={() =>
                            this.setState({
