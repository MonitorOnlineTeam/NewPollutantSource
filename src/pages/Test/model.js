  ...
  effects: {
    ...
    * getPointTableData({
      ...
      if (result.IsSuccess) {
        ...
        const pollutantType = type == 1 ? yield select(state => state.test.waterList) : yield select(state => state.test.gasList)
    ...
    * getPointChartData({
      ...
      if (result.IsSuccess) {
        // const tableList = yield select(state => state.test.tableList);
    ...
    * updateChartData({ payload }, { select, update, put }) {
      ...
      const chartData = yield select(state => state.test.chartData);
      const xAxisData = [];
