  ...
  onSubmitForm() {
    ...
    setTimeout(() => {
      dispatch({
        ...
        callback: () => {
          successCallback && successCallback(searchForm);
        },
      });
    }, 0);
  }
      ...
      switch (item.type) {
        ...
        case '三级行政区下拉框':
          ...
          element = (
            <SdlCascader
              ...
              data={item.value}
              selectType={
                labelText === '行政区' && !this.props.isCoustom
                  ? item.DF_OtherOptions
                  : this.props.isCoustom
        ...
        case '日期范围':
          ...
          if (item.DF_QUERY_TIME_TYPE && item.LIST_TIME) {
            initialValue = [
              moment().subtract(item.LIST_TIME * 1, item.DF_QUERY_TIME_TYPE),
              moment(),
            ];
