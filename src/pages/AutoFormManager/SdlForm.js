...
class SdlForm extends PureComponent {
    ...
    if (format === 'YYYY-MM-DD') {
      return (
        <DatePicker
          ...
          style={{ width: '100%' }}
          disabledDate={
            item.fullFieldName === 'dbo.T_Bas_CommonPoint.Col10' ? this.disabledDate : null
          }
        />
  ...
  renderFormItem() {
    ...
    return formItems.map((item, index) => {
      ...
      let stationPlaceStr =
        '命名规则：空气站所在地级市名称+空气站、直辖市名+空气站，例如郑州市空气站、北京市空气站';
      // 判断类型
      switch (item.type) {
        case '文本框':
          ...
          } else if (item.labelText === '登录名' && configId === 'UserInfoAdd') {
            //用户管理 基本信息
          ...
          element = (
            <Input
              ...
              title={item.labelText === '大气站名称' && stationPlaceStr}
              allowClear
            />
          );
          break;
      ...
      if (element) {
        ...
        return (
          <Col
            ...
            span={colSpan}
            style={{ display: item.isHide == 1 ? 'none' : '' }}
          >
  ...
  render() {
    ...
    return (
      <Fragment>
        <Spin spinning={this.props.isModal && this.props.isEdit ? this.props.fileLoading : false}>
          {' '}
          {/*弹框编辑的加载状态loading */}
          {this.renderContent()}
        </Spin>
