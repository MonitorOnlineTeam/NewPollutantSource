...
const Index = (props) => {
  ...
  return (
    <div className={styles.quesDetailSty} style={{padding:isMobile? '16px 12px' : 0,height:isMobile? '100vh': '100%',backgroundColor:'#fff'}}>
         <Spin spinning={questionDetialLoading} active style={{height:isMobile? '100vh':'100%'}}>
       <div style={{textAlign:'center',fontWeight:'bold'}}>{questionDetail.QuestionName}</div>
       <div style={{textAlign:'center',color:'rgb(194,194,194)',paddingTop:8}}><span>创建人：{questionDetail.CreateUserName}</span> <span style={{paddingLeft:24}}>创建时间：{questionDetail.CreateTime}</span></div>
