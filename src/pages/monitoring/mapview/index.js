...
class MapView extends Component {
  ...
  renderMarker = extData => (
      ...
      onClick={() => {
        if (extData) {
          ...
          if (this.state.displayType === 1) {
            ...
            if (
              extData.position.MonitorObjectType == 2 ||
              extData.position.MonitorObjectType == 4
            ) {
              // 监测站
              newState = {
                // infoWindowVisible: true,
          ...
          this.setState(
            ...
            () => {
              ...
              const isAirOrSite =
                extData.position.MonitorObjectType == 2 || extData.position.MonitorObjectType == 4;
              // const isAirOrSite = extData.position.MonitorObjectType == 2 || extData.position.MonitorObjectType == 4;
              !isAirOrSite &&
                newState.displayType == 1 &&
                this.randomMarker(extData.position.children);
              extData.position.PollutantType &&
    ...
    >
      ...
      // </Popover>
      extData.position && (
        <div content={extData.position.title}>
          {// this.state.displayType === 0 ?
          //   <EntIcon style={{ fontSize: 40 }} /> :
          //   (extData.position.PollutantType === "2" ?
          //     <GasIcon
          ...
          pointEl = (
            <>
              ...
              <div
                ...
                style={{ left: '-11px', top: -12, display: isShow }}
              ></div>
            </>
          );
        }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      this.props.allEntAndPointList !== nextProps.allEntAndPointList &&
      nextProps.allEntAndPointList.length === 1
    ) {
      const timer = setInterval(() => {
        if (_thismap) {
          this.setState({
  ...
  render() {
    ...
    const option = {
      ...
      yAxis: {
        ...
        name: this.state.chartTitle
          ? this.state.chartTitle
          : this.props.tableList.length && this.props.tableList[0].title,
        axisLabel: {
          formatter: '{value}',
        },
      },
    ...
    const airOption = {
      // color: ['#3398DB'],
      title: {
        text: this.state.currentDescItem.label
          ? `${this.state.currentDescItem.label} 24小时${this.state.airShowType}柱状图`
          : '24小时AQI柱状图',
        textStyle: {
          color: 'rgba(0, 0, 0, 0.75)',
          fontSize: 15,
    ...
    return (
      ...
      <div className={styles.mapWrapper} style={mapWrapperStyle}>
                ...
                if (this.state.currentEntInfo == entInfo[0]) {
                  ...
                  if (entInfo[0].MonitorObjectType == 2 || entInfo[0].MonitorObjectType == 4) {
                    ...
                    if (this.state.displayType == 1) {
                      ...
                      _thismap.setZoomAndCenter(this.state.zoom, [
                        entInfo[0].Longitude,
                        entInfo[0].Latitude,
                      ]);
        ...
        <div
          id="contentWrapper"
          style={{ height: 'calc(100vh - 64px)', marginLeft: '320px', position: 'relative' }}
        >
          {displayType == 1 && (
            <Button
              type="primary"
              onClick={this.showEntMarker}
          ...
          >
            ...
            >
              {
                <div className={styles.pointInfoWindow}>
                  {this.props.pointLoading ||
                  this.props.chartLoading ||
                  this.props.pollutantLoading ||
                  this.props.airChartLoading ? (
                    <Spin
                      style={{
                        width: '100%',
                        height: '310px',
                            ...
                            title={
                              ...
                              <div>
                                ...
                                <span style={{ fontWeight: 'normal', fontSize: 13 }}>
                                  {this.props.monitorTime
                                    ? `监控时间：${this.props.monitorTime}`
                                    : ''}
                                </span>
                              </div>
                            }
            ...
            <InfoWindow
              ...
              size={{
                width: 480,
                height: this.state.currentPointInfo.MonitorObjectType == '4' ? 380 : 440,
              }}
              // size={{ width: 480 }}
              style={{ maxHeight: 524 }}
              closeWhenClickMap
                  ...
                  <Descriptions
                    title={
                      <div className={styles.airDescBox}>
                        ...
                        <span>
                          {this.props.monitorTime ? `监控时间：${this.props.monitorTime}` : ''}
                        </span>
                      </div>
                    }
                    size="small"
          ...
          {/* 空气指数图例 */}
          {(this.state.currentPointInfo.MonitorObjectType == '2' ||
            this.state.currentPointInfo.MonitorObjectType == '4') && (
          ...
          >
            ...
            >
              ...
              {menuNameList.includes('超标处置') &&
                this.state.currentPointInfo.PollutantType != '5' && (
                  <TabPane tab="超标处置" key="3">
                    <AlarmRecord
                      DGIMN={currentKey}
                      EntCode={this.state.currentPointInfo.EntCode}
                      initLoadData
                      dataHeight="calc(100vh - 450px)"
                      style={{ maxHeight: modalHeight + 52, height: 'calc(100vh - 366px)' }}
                    />
                  </TabPane>
                )}
              {menuNameList.includes('异常数据') && (
                <TabPane tab="异常数据" key="4">
                  <RecordEchartTable
                    DGIMN={currentKey}
                    initLoadData
                    ...
                    <div>
                      <Descriptions title={infoWindowData.pointName}>
                        ...
                        <Descriptions.Item label="运维负责人">
                          {infoWindowData.operationPerson}
                        </Descriptions.Item>
                        <Descriptions.Item label="污染物类型">
                          {infoWindowData.pollutantType}
                        </Descriptions.Item>
                      </Descriptions>
                    </div>
                  </div>
                  ...
                  {this.state.currentPointInfo.PollutantType !== '5' && (
                    <Descriptions title="企业信息">
                      <Descriptions.Item label="企业名称">
                        {infoWindowData.entName}
                      </Descriptions.Item>
                      <Descriptions.Item label="行业">
                        {infoWindowData.industryName}
                      </Descriptions.Item>
                      <Descriptions.Item label="控制级别名称">
                        {infoWindowData.attentionName}
                      </Descriptions.Item>
                      <Descriptions.Item label="环保负责人">
                        {infoWindowData.EnvironmentPrincipal}
                      </Descriptions.Item>
                      <Descriptions.Item label="移动电话">
                        {infoWindowData.entphone}
                      </Descriptions.Item>
                      <Descriptions.Item label="企业地址">
                        {infoWindowData.entadress}
                      </Descriptions.Item>
                    </Descriptions>
                  )}
                  {infoWindowData.longitude && infoWindowData.latitude && (
                    <SdlMap
                      mode="map"
                      ...
                      handlePolygon
                      style={{
                        height: this.state.currentPointInfo.PollutantType !== '5' ? 300 : 430,
                      }}
                      zoom={12}
                    />
                  )}
