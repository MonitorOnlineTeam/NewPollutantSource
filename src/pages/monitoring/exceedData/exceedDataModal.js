      ...
      .then(() => {
        ...
        if (this.props.PollutantByType.length > 0) {
          ...
          this.props.PollutantByType.map(item => {
            if (
              item.PollutantCode == exceedPollutant ||
              item.PollutantCode == `zs${exceedPollutant}`
            ) {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        PollutantByType.map(item => {
          ...
          let Max = null;
          if (
            values[item.PollutantCode] != '' &&
            values[item.PollutantCode].toString() == item.PollutantCode
          ) {
            let pollution = {};
            ...
            let obj = {
              ...
              content: (
                <SdlTable
                  ...
                  scroll={{
                    y: this.state.entType == '1' ? 'calc(100vh - 680px)' : 'calc(100vh - 730px)',
                  }}
                  // pagination={
                  //     {
    ...
    return (
      <>
        <Form onSubmit={this.handleSummit} layout="inline">
          ...
          <Form.Item>
            <Button style={{ marginRight: 10 }} htmlType="submit" onClick={this.exportReport}>
              <ExportOutlined />
              导出
            </Button>
          </Form.Item>
            ...
            <TabPane tab={this.state.entType == '1' ? '废水' : '废气'} key="1" closable={false}>
              {
                <SdlTable
                  ...
                  scroll={{
                    x: scrollWith,
                    y: this.state.entType == '1' ? 'calc(100vh - 600px)' : 'calc(100vh - 650px)',
                  }}
                  loading={loading}
                  pagination={{
                    showSizeChanger: true,
        ...
        <div id="siteParamsPage" className={style.cardTitle}>
          ...
          >
            <div style={{ marginBottom: 10 }}>
              ...
              <Button type="primary" style={{ marginRight: 10 }} onClick={this.ExButtonCountHandle}>
                {' '}
                查询
              </Button>
