...
class CommonChart extends Component {
  ...
  getEchart = () => {
    ...
    if (option) {
      ...
      return (
        <ReactEcharts
          ref={reactEcharts => {
            this.echartsReact = reactEcharts;
          }}
          theme="light"
          option={option}
          lazyUpdate
  ...
  pollutantClick = e => {
    this.dashboardClick(
      this.props.pollutantlist[e].PollutantCode,
      this.props.pollutantlist[e].PollutantName,
      this.props.pollutantlist[e].Unit,
    );
  };
  ...
  getLastestData = () => {
    ...
    if (pollutantlist) {
      ...
      pollutantlist.map((item, key) => {
        if (paramsInfo) {
          const pollutantParam = paramsInfo.find(
            param => param.pollutantCode == item.PollutantCode,
          );
          if (pollutantParam) {
            // 定义超标异常标识
            let flag = 2;
                ...
                tab={
                  <div size="small" className={styles.maincard} bordered={false}>
                    <p style={{ marginBottom: 1 }}>
                      <span style={{ color: '#00000073' }}>{pollutantParam.pollutantName}</span>
                      <Popover
                        placement="rightTop"
                        title="详细参数"
                        content={this.getDeials(pollutantParam.pollutantParamInfo)}
                        trigger="hover"
                      >
                        <ExclamationCircleOutlined style={{ float: 'right', fontSize: '15px' }} />
                      </Popover>
                    </p>
                    <p style={{ marginBottom: 1 }}>
                      浓度：
                      <span
                        style={{ color: flag === '0' ? 'red' : flag === '1' ? '#ffbe00e3' : '' }}
                        className={styles.cardfonttype}
                      >
                        {pollutantParam.value}
                      </span>
                      {pollutantParam.state ? (
                        pollutantParam.state === '1' ? (
                          <span>
                            <Xiangshang style={{ fontSize: 20, height: 18 }} />
                          </span>
                        ) : pollutantParam.state === '2' ? (
                          <span>
                            {' '}
                            <Xiangxia style={{ fontSize: 20, height: 18 }} />
                          </span>
                        ) : (
                          ''
                        )
    ...
    return (
      <div style={{ backgroundColor: '#ffffff' }}>
        ...
        >
          ...
          <Tabs onChange={this.pollutantClick}>{this.getLastestData()}</Tabs>
          {/* <OfflineData
                        activeKey={0}
                        loading={dataloading}
