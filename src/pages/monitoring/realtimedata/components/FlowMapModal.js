 ...
 * @Last Modified time: 2023-03-08 15:24:13
...
class FlowMapModal extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (JSON.stringify(this.props.paramsInfo) !== JSON.stringify(prevProps.paramsInfo) && this.props.visible === true) {
  ...
  // 渲染canvas
  renderCanvas = () => {
    const { paramsInfo, currentPointData: { yNum, xNum, height, width, FlowMeterType } } = this.props;
    if (FlowMeterType == 2) {
      // this.setState({
      // }, () => {
  ...
  renderFlows = (data) => {
    const { paramstatusInfo, paramsInfo, currentPointData: { yNum, xNum, height, width, FlowMeterType } } = this.props;
  ...
  render() {
    ...
    const { visible, onCancel, paramsInfo, CEMSOpen, QCStatus, valveStatus, CO2Rate, CO2SampleGasValue, O2SampleGasValue, wrapperStyle, vertical, scale } = this.props;
