...
class DataTagTable extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      dataSource: [],
      visible: false,
      prevQuarterStart: moment()
        .quarter(moment().quarter() - 1)
        .startOf('quarter')
        .valueOf(),
      prevQuarterEnd: moment()
        .quarter(moment().quarter() - 1)
        .endOf('quarter')
        .valueOf(),
  ...
  isQualifiedTime = time => {
    ...
    // 传入的时间是否是在上个季度内
    let timeIsInsideQuarter =
      moment(time).isBefore(this.state.prevQuarterEnd) &&
      moment(time).isAfter(this.state.prevQuarterStart);
          ...
          render: (text, record, index) => {
            ...
            return (
              <div className={styles.tdItemBox}>
                ...
                {isShowCheckBox && (
                  <Checkbox
                    checked={record[item.PollutantCode + '_checked']}
                    onChange={e => {
                      ...
                      if (e.target.checked) {
                        ...
                        // 选中后新增一条数据
                        checkedRowList[
                          currentListItem.DataGatherCode + '/' + currentListItem.MonitorTime
                        ] = currentListItem;
                      } else {
                        currentListItem['checkedNum'] = --checkedNum;
                        // 如果行内没有选中的单元格，删除对应的key
                        currentListItem['checkedNum'] === 0 &&
                          delete checkedRowList[
                            currentListItem.DataGatherCode + '/' + currentListItem.MonitorTime
                          ];
                      }
                      this.setState({
        ...
        columns: [
          {
            ...
            render: (text, record, index) => {
              return (
                <Checkbox
                  indeterminate={
                    record.checkedNum > 0 && record.checkedNum != nextProps.columnsData.length
                  }
                  onChange={e => {
                    this.onCheckRowChange(e, record, index);
                  }}
  ...
  onCheckRowChange = (e, record, index) => {
    ...
    let checkedRowList = { ...this.state.checkedRowList };
    let isShowCheckBox =
      moment(currentListItem.time).isBefore(this.state.prevQuarterEnd) &&
      moment(currentListItem.time).isAfter(this.state.prevQuarterStart);
    if (e.target.checked) {
      // 全选行
      ...
      if (!err) {
        ...
        for (const item in checkedRowList) {
          ...
          for (const key in checkedRowList[item]) {
            ...
            checkedRowList[item]['MonitorTime'] =
              this.props.dataType === 'mins'
                ? moment(checkedRowList[item]['MonitorTime']).format('YYYY-MM-DD HH:mm:00')
                : moment(checkedRowList[item]['MonitorTime']).format('YYYY-MM-DD HH:00:00');
            // itemObj[keyFlag + "_Flag"] = values.status
          }
          postData.Data.push({
            ...checkedRowList[item],
  ...
  render() {
    ...
    return (
      <>
        <Alert
          message={
            <p
              ...
              style={{ textAlign: 'left', color: '#6f6868', fontSize: 12 }}
            >
        ...
        <SdlTable columns={columns} {...this.props} dataSource={dataSource} />
        <Modal
          title="修改数据状态"
