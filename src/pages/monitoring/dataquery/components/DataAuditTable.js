...
class DataAuditTable extends Component {
    ...
    return (
      <div>
        <p style={{ marginBottom: 4 }}>
          数据将被修改为<span style={{ color: color }}>{text}</span>，请填写{' '}
        </p>
        <label className="ant-form-item-required">修改原因：</label>
        <Form>
          <Form.Item>{getFieldDecorator('msg', {})(<Input style={{ marginTop: 4 }} />)}</Form.Item>
        </Form>
      </div>
    );
  };
  ...
  changeValueStatus = (record, pollutantCode, flag) => {
    ...
    let time =
      this.props.dataType === 'hour'
        ? record.MonitorTime + ':00:00'
        : record.MonitorTime + ' 00:00:00';
    this.props.dispatch({
      ...
      this.setState({
        columns: [
          ...
          },
          {
            ...
            // fixed: "left",
            render: (text, record) => {
              return this.props.dataType === 'hour'
                ? moment(text).format('YYYY-MM-DD HH') + '时'
          ...
          (dataType === 'hour' ? (
            <Alert
              message={
                ...
                >
                  H：有效数据不足 BB：连接不良 B：运行不良 W：等待数据恢复 HSp：数据超上限
                  LSp：数据超下限 PS：跨度检查 PZ：零点检查 AS：精度检查 CZ：零点校准 CS：跨度校准
                  RM：自动或人工审核为无效 E或D：站点有效个数不足 LT：缺仪器温度 LP：缺仪器压力
                  NT：无温度值 NP：无压力值
                </p>
              }
              type="warning"
