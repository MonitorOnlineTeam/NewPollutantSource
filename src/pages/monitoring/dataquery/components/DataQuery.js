...
class DataQuery extends Component {
  ...
  getpointpollutants = dgimn => {
    const { pollutantlist, historyparams } = this.props;
    this.props.dispatch({
  ...
  reloaddatalist = historyparams => {
    const { dispatch, dataStatus } = this.props;
    let IsSupplyData =
    ...
    let queryParams = {};
    if (this.props.pollutantCode) {
      queryParams = {
        pollutantCodes: this.props.pollutantCode,
  ...
  loaddata = () => {
    ...
    if (displayType === 'chart') {
      if (option) {
        return (
          <ReactEcharts
            showLoading={dataloading || loadingPollutant}
            theme="light"
            option={option}
            lazyUpdate
            notMerge
  ...
  render() {
    ...
    return (
      <div>
        <Card
          bordered={tableHeight ? false : true}
          headStyle={{ display: hideTitle !== undefined ? 'none' : 'block' }}
            ...
            <div>
              ...
              <div style={{ marginTop: 8 }}>
                <Form layout="inline">
                  ...
                  {Type === '5' && isShowSearchDataType && (
                    <Form.Item style={{ f: 5 }}>
                      <Select
                        style={{ width: 100 }}
                        value={searchDataType}
                        onChange={value => {
                          this.setState({ searchDataType: value });
                        }}
                      >
