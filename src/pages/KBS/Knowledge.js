...
class Knowledge extends PureComponent {
  ...
  render() {
    ...
    // 处理系统名称，替换"平台"或"系统"为"资源中心"
    const getSystemName = (name) => {
      if (!name) return '';
      return name.replace(/(平台|系统)/g, '资源中心');
    }

    return (
      <div>
        <div className={styles.banner}>
          <h1>{getSystemName(configInfo.SystemName)}</h1>
          <div className={styles.inputContainer}>
            ...
            <Button type="link" onClick={() => this.getKBSData()}>
              搜索
            </Button>
          </div>
        </div>
        <Row style={{ paddingTop: '240px', justifyContent: 'center' }}>
          {Object.keys(KBSData).map(key => {
            return (
              <Col className={styles.cardItem} style={{ width: 300 }}>
                <Card
                  size="small"
                      ...
                      this.setState(
                        {
                          searchParams: [
                            {
                              Key: 'dbo__T_Bas_Repository__RepositoryType',
                              Value: key,
                              Where: '$=',
                            },
                          ],
