...
class Index extends React.Component {
  constructor(props) {
    ...
    this.columns = [
      ...
      // },
      {
        ...
        text == 2 ? <span style={{ color: green.primary }}>已下发</span> : text == "下发中" ? <Spin size="small" /> : <span style={{ color: red.primary }}>已保存</span>}</>,
      },
      {
        title: '操作',
        dataIndex: 'save',
  ...
  isSave = (row, name, index) => {
    if (name === "取消") {
      ...
      //  计算时间差
      const date1 = this.props.maxTime;
      let flag = true;
      if (date1) {
        const date2 = moment(addParams.Date + " " + addParams.Time + ":00");
        let date3 = date2.diff(date1, 'minute');
        if (date3 < interval) {
          flag = false;
          message.error('每个监测项目，质控间隔时间需大于或等于' + interval + '分钟，请重新选择时间！')
        }
      }
      if (flag) { 
        dispatch({
  ...
  render() {
    console.log('maxTime=', this.props.maxTime)
