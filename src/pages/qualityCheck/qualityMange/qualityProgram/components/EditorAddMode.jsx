...
class EditorAddMode extends React.Component {
  ...
  uploadSet = (type) => {
    ...
    return {
      // name: 'file',
      action: "/api/rest/PollutantSourceApi/QCCheckConfig/UploadFiles",
      onChange(info) {
  ...
  defaultFileList = (type) => {
    ...
    if (echoType != "添加") {
      if (type == "explain") {
        let filePath = editEchoData.ProgrammeFileName.split('/');
        let fileName = filePath[filePath.length - 1];
        return [{
          uid: editEchoData.ProgrammeFile,
          name: fileName,
          status: 'done',
          url: `${editEchoData.ProgrammeFileName}`,
        }]
      } else {
        let filePath = editEchoData.DetailsFileName.split('/');
        let fileName = filePath[filePath.length - 1];
        return [{
          uid: editEchoData.DetailsFile,
          name: fileName,
          status: 'done',
          url: `${editEchoData.DetailsFileName}`,
        }]
      }
    } else {
      return [];
