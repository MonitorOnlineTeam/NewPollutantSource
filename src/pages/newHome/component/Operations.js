...
class Operations extends PureComponent {
  ...
  componentDidMount() {
    ...
    this.zr.on('click', (...rest) => {
      var indexArr = this.echartsInstance.convertFromPixel({ seriesIndex: 0 }, [
        rest[0].offsetX,
        rest[0].offsetY,
      ]);
  ...
  option = () => {
    const {
      operationAnalysis: {
        ...
        verificationTestUnfinished,
        maintenanceRepairComplete,
        cooperationInspectionComplete,
        matchingComplete,
        manualComparisonComplete,
        calibrationComplete,
        onSiteInspectionComplete,
        maintenanceRepairUnfinished,
        cooperationInspectionUnfinished,
        matchingUnfinished,
        manualComparisonUnfinished,
        calibrationUnfinished,
        onSiteInspectionUnfinished,
      },
    } = this.props;
    return {
      tooltip: {
  ...
  render() {
    ...
    return (
      <div className={styles['group-item']}>
        <div className={styles['item-title']}>运维分析</div>
        <div className={styles['item-content']}>
          <div className={styles.innerTitle}>
            ...
            <span>
              <i style={{ backgroundColor: '#0edaad' }}></i>未完成
            </span>
          </div>
          <ReactEcharts
            option={this.option()}
