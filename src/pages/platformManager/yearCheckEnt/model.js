...
export default Model.extend({
  ...
  effects: {
    ...
    *ExportAnnualAssessmentEnt({ payload }, { call, put, update, select }) {
      //企业
      const { RegionCode, pollutantType, entCode, qutletQueryPar, AssessYearStr } = yield select(
        state => state.yearCheckEnt,
      );
      let body = {
        RegionCode: RegionCode,
        AssessYear: AssessYearStr,
    ...
    *GetAnnualAssessmentEntAndPoint({ payload }, { call, put, update, select }) {
      //企业
      const { AssessYearStr } = yield select(state => state.yearCheckEnt);
      let body = {
        AssessYear: AssessYearStr,
      };
