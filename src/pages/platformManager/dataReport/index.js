...
export default class MonitorTarget extends Component {
  ...
  showDeleteConfirm = (row, id) => {
    ...
    const { dispatch, match } = this.props;
    if (
      moment(row['dbo.T_Bas_DataReporting.MonitorTime']).format('YYYY-MM') ==
  ...
  render() {
    const {
      ...
      tableInfo,
      match: {
        params: { configId, monitortime, entcode },
      },
      dispatch,
      selectmonth,
      reportwhere,
          ...
          <Card className={styles.contentContainer}>
            <Row>
              ...
              <Col xxl={6} xl={6} lg={8}>
                <FormItem {...formLayout} label="" style={{ width: '100%' }}>
                  <Button type="primary" style={{ margin: '0 10px' }} onClick={this.Serach}>
                    查询
                  </Button>
                </FormItem>
              </Col>
            ...
            <AutoFormTable
              ...
              onAdd={() => {
                dispatch(
                  routerRedux.push(
                    `/Intelligentanalysis/SewagePlant/DataReportingAdd/${configId}/${null}/${selectmonth}/${entcode}`,
                  ),
                );
              }}
                    ...
                    <a
                      onClick={() => {
                        if (
                          ...
                          dispatch(
                            routerRedux.push(
                              `/Intelligentanalysis/SewagePlant/DataReportingAdd/${configId}/${
                                row['dbo.T_Bas_DataReporting.ID']
                              }/${selectmonth}/${entcode}`,
                            ),
                          );
                        } else {
