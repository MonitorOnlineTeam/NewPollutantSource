...
@connect(({ manualupload, loading }) => ({
  ...
  reason: manualuploadauto.reason,
}))
@connect()
    ...
    if (value) {
      ...
      if (item) {
        ...
        addSelectPollutantData
          .filter(n => n.PollutantCode == PollutantCode)
          .map(item => {
            unit = item.Unit;
          });
      }
        ...
        <Form onSubmit={this.handleSubmit}>
          <Row gutter={24}>
            <Col xs={2} sm={6} md={12} lg={12} xl={12} xxl={12}>
              <FormItem {...formItemLayout} label={'污染物类型'}>
                {getFieldDecorator('pollutantType', {
                  initialValue: addSelectPollutantData
                    ? addSelectPollutantData[0].PollutantTypeName
                    : null,
                })(
                  <Select disabled={true}>
                    {addSelectPollutantData ? (
                      <Option key={addSelectPollutantData[0].PollutantTypeCode}>
                        {addSelectPollutantData[0].PollutantTypeName}
                      </Option>
                    ) : null}
                  </Select>,
            ...
            <Col xs={2} sm={6} md={12} lg={12} xl={12} xxl={12}>
              <FormItem {...formItemLayout} label={'污染物名称'}>
                {getFieldDecorator('PollutantCode', {
                  ...
                  <Select
                    ...
                    disabled={isExists}
                  >
                    {addSelectPollutantData !== null
                      ? addSelectPollutantData.map(item => (
                          <Option key={item.PollutantCode + '--' + item.PollutantName}>
                            {item.PollutantName}
                          </Option>
                        ))
                      : null}
          ...
          <Row gutter={24}>
            <Col xs={2} sm={6} md={12} lg={12} xl={12} xxl={12}>
              <FormItem {...formItemLayout} label={'检测时间'}>
                {getFieldDecorator('MonitorTime', {
                  initialValue: isExists
                    ? moment(item.MonitorTime === null ? Date.now() : item.MonitorTime)
                    : '',
                  rules: [
                    {
                      required: true,
            ...
            <Col xs={2} sm={6} md={12} lg={12} xl={12} xxl={12}>
              <FormItem {...formItemLayout} label={'浓度'}>
          ...
          <Row gutter={16} style={{ marginTop: 8 }}></Row>
          <Row gutter={16} style={{ marginTop: 8 }}>
            <Col xs={2} sm={6} md={12} lg={12} xl={12} xxl={12} style={{ display: 'none' }}>
              <FormItem {...formItemLayout} label={'点编号'}>
