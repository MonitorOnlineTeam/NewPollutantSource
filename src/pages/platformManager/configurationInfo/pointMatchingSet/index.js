      ...
      render: (text, record) => {
        return <span>
          ...
          <Fragment> <Tooltip title="删除">
            <Popconfirm title="确定要删除匹配信息吗？" style={{ paddingRight: 5 }} onConfirm={() => { del(record) }} okText="是" cancelText="否">
              <a><DelIcon /></a>
            </Popconfirm>
          </Tooltip>
          </Fragment>
    ...
    render: (text, record) => {
      return <span>
        <Fragment> <Tooltip title="选择">
          <Popconfirm
            title="确定要选择吗？"
            onConfirm={() => { choice(record) }}
          >
           <a><CheckCircleOutlined style={{ fontSize: 16 }} /></a>
          </Popconfirm>
