...
const dvaDispatch = dispatch => {
  return {
    addOrUpdateEquipmentParametersInfo: (payload, callback) => {
      //修改 or 添加
      dispatch({
    ...
    getEquipmentParametersInfo: (payload, callback) => {
      //参数列表
      dispatch({
    ...
    deleteEquipmentParametersInfo: (payload, callback) => {
      //删除
      dispatch({
  ...
  const edit = async record => {
    try {
      ...
      form.setFieldsValue({
        ...
        ['DetectionLimit' + (() => record.ID)()]: record.DetectionLimit
          ? record.DetectionLimit
          : '',
  ...
  const save = async record => {
    try {
      ...
      let pass =
        ...
        EquipmentParametersCode.toString() &&
        Range1Min.toString() &&
        Range1Max.toString();
      ...
      if (pass) {
        ...
        props.addOrUpdateEquipmentParametersInfo(payload, () => {
          setTableLoading(false);
          const EquipmentParametersName =
            record.type == 'add'
              ? parametersList.filter(
                  item => item.ChildID === row[`EquipmentParametersCode${record.ID}`],
                )[0].Name
          ...
          props.getEquipmentParametersInfo({ DGIMN: DGIMN }, res => {
            setTableLoading(false);
            setData(res);
          });
        });
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        ...
        return record.Range2Min != null && record.Range2Max != null
          ? `${record.Range2Min} ~ ${record.Range2Max}`
          : '';
        // }
      },
    },
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        if (record.type == 'add') {
          return (
            <>
              <Typography.Link onClick={() => save(record)}>添加</Typography.Link>
              <Typography.Link
                onClick={() => handleAddDelete(record.ID)}
                style={{ paddingLeft: 8 }}
              >
                删除
              </Typography.Link>
        ...
        return record.type == 'modify' ? (
          ...
          <>
            ...
            <Popconfirm title="确定要删除?" onConfirm={() => handleDelete(record.ID)}>
              <Typography.Link style={{ paddingLeft: 8 }}>删除</Typography.Link>
            </Popconfirm>
          </>
        );
      },
  ...
  const mergedColumns = columns.map(col => {
    ...
    return {
      ...
      onCell: record => ({
        record,
        inputType:
          col.dataIndex === 'Range1' || col.dataIndex === 'Range2'
            ? 'range'
            : col.dataIndex === 'EquipmentParametersCode'
            ? 'select'
            : 'text',
    ...
    <Card title={'量程设定'}>
      <Form form={form} layout={'horizontal'} component={false}>
        <SdlTable
          ...
          columns={
            type === 'smoke'
              ? mergedColumns.filter(item => item.dataIndex !== 'DetectionLimit')
              : mergedColumns
          }
