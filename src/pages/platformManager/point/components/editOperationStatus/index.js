...
const dvaDispatch = dispatch => {
  return {
    ...
    updatePointOprationStatus: (payload, callback) => {
      //修改点位运维状态
      dispatch({
    ...
    getOprationStatusList: (payload, callback) => {
      //修改记录
      dispatch({
    ...
    getAutoFormData: (payload, callback) => {
      //监测点列表
      dispatch({
...
const Index = props => {
  ...
  const {
    ...
    tableTotal,
    tableLoading,
    updatePointOprationStatusLoading,
    data,
    pointConfigId,
    pointDataWhere,
  } = props;
    ...
    return (
      ...
      >
        <Form.Item
          name="Status"
          label="运维状态"
          rules={[{ required: true, message: '请选择运维状态！' }]}
        >
          <Radio.Group
            onChange={e => {
        ...
        {status === '1' && (
          <Form.Item
            name="PauseReason"
            label="暂停原因"
            rules={[{ required: true, message: '请选择暂停原因！' }]}
          >
            {pauseReasonLoading ? (
              <Spin size="small" />
            ) : (
              <Select allowClear placeholder="请选择">
                {pauseReasonList.map(item => (
                  <Option value={item.ChildID}>{item.Name}</Option>
                ))}
              </Select>
            )}
          </Form.Item>
