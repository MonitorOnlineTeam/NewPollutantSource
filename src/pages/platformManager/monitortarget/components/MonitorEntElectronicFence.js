...
const dvaDispatch = dispatch => {
  return {
    ...
    addOrUpdOperationSignRadiusInfo: (payload, callback) => {
      //添加修改
      dispatch({
    ...
    getOperationSignRadiusInfo: (payload, callback) => {
      //获取
      dispatch({
...
const Index = props => {
  ...
  return (
    <Modal
      ...
      title={title}
      visible={visible}
      onOk={monitorEntElectronicFenceOk}
      onCancel={() => {
        props.onCancel();
      }}
      confirmLoading={props.addUpdataLoading || props.getDataLoading}
      destroyOnClose
    >
      <Spin spinning={props.getDataLoading} size="small">
        <Form name="basic" form={form}>
          <Form.Item
            label="电子围栏半径（KM）"
            name="radius"
            rules={[{ required: true, message: '请输入电子围栏半径!' }]}
          >
