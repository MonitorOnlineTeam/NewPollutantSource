...
const dvaDispatch = dispatch => {
  return {
    ...
    getEntProjectRelationList: payload => {
      //监测运维列表
      dispatch({
    ...
    updateOrAddProjectRelation: (payload, callback) => {
      //修改 or 添加
      dispatch({
    ...
    deleteOperationPoint: (payload, callback) => {
      //删除
      dispatch({
    ...
    exportEntProjectRelationList: payload => {
      //导出
      dispatch({
    ...
    calibrationCycle: (payload, callback) => {
      // 校准
      debugger;
      dispatch({
        type: 'autoForm/getAutoFormData',
    ...
    inspectionCycle: (payload, callback) => {
      //巡检频次 巡检类型
      dispatch({
        type: 'autoForm/getAutoFormData',
        payload: {
    ...
    SystemCalibrationCycle: (payload, callback) => {
      // 全系统校准派单频次
      dispatch({
        type: 'autoForm/getAutoFormData',
        payload: {
...
const Index = props => {
  ...
  const [systemCalibrationCycleList, setSystemCalibrationCycleList] = useState([]);
  ...
  const initData = () => {
    ...
    props.SystemCalibrationCycle({}, data => {
      //全系统校准派单频次
      if (data && data[0]) {
        const list = data.filter(item => item['dbo.T_Cod_OperationCycle.Status'] == 1);
        setSystemCalibrationCycleList(list);
      }
    });
  };
  ...
  const columns = [
    ...
    // },
    {
      ...
      width: 180,
      sorter: (a, b) =>
        moment(a.operationBeginTime).valueOf() - moment(b.operationBeginTime).valueOf(),
    },
    {
      title: '运维合同结束日期',
    ...
    },
    {
      ...
      render: (text, record) => {
        ...
        return (
          <span>
            <Fragment>
              <Tooltip placement={placement} title={flag ? `${tipText}` : '编辑'}>
                {' '}
                <a
                  style={{ ...disSty }}
                  onClick={() => {
            ...
            <Fragment>
              {' '}
              <Tooltip placement={placement} title={flag ? `${tipText}` : '删除'}>
                <Popconfirm
                  title="确定要删除此条信息吗？"
                  disabled={flag}
  ...
  const edit = async record => {
    setFileList1([]);
    if (record.uploadInfo && record.uploadInfo[0]) {
      // 运维接收确认单附件
    ...
    form2.setFieldsValue({
      ...
      PorjectID: record.projectID,
      OperationCompany: record.companyID,
      InspectionCycel: record.inspectionCycel,
      CalibrationCycle: record.calibrationCycle,
      ParameterCheck: record.parameterCheck,
      RegionCode: record.regionCode,
      BeginTime: record.actualBeginTime ? moment(record.actualBeginTime) : undefined,
      EndTime: record.actualEndTime ? moment(record.actualEndTime) : undefined,
    });
    setChoiceData(record.projectCode);
    setType('edit');
    setFromVisible(true);
  ...
  const onModalOk = async () => {
    ...
    try {
      ...
      props.updateOrAddProjectRelation(
        {
          ...values,
          BeginTime: values.BeginTime && values.BeginTime.format('YYYY-MM-DD 00:00:00'),
          EndTime: values.EndTime && values.EndTime.format('YYYY-MM-DD 23:59:59'),
  ...
  const operationDataSource =
    operationInfoList['OperationMaintenanceEnterprise'] &&
    operationInfoList['OperationMaintenanceEnterprise'].dataSource
      ? operationInfoList['OperationMaintenanceEnterprise'].dataSource
      : [];
  ...
  cycleList[0] &&
    cycleList.map(item => {
      const code = item['dbo.T_Cod_OperationCycle.Code'],
        id = item['dbo.T_Cod_OperationCycle.ID'];
      //  if(code==2){
      //    form2.setFieldsValue({
      //      CalibrationCycle:code
      //    })
  ...
  return (
    <div className={styles.entOperationInfo}>
      ...
      <Modal
        ...
        // centered
      >
        <Form name="basic2" form={form2}>
          <Row>
            <Col span={12}>
              <Form.Item
                label="监测点列表"
                name="DGIMN"
                rules={[{ required: true, message: '请选择监测点列表!' }]}
              >
                <Select
                  mode={type === 'edit' ? null : 'multiple'}
                  optionFilterProp="children"
                  showArrow
                  maxTagPlaceholder="..."
                  maxTagTextLength={4}
                  ...
                  content={
                    <>
                      ...
                      <SdlTable
                        ...
                        style={{ width: 800 }}
                        loading={projectNumListLoading}
                        bordered
                        dataSource={projectTableDatas}
      ...
      <Form.Item label="省份名称" name="RegionCode" rules={[  { required: true, message: '请选择省份名称!',  },]} >
       <RegionList     levelNum={1} selectType = '1,是' style={{width:'100%'}}/>
            ...
            <Col span={12}>
              <Form.Item
                label="巡检派单频次"
                name="InspectionCycel"
                rules={[{ required: true, message: '请选择巡检频次!' }]}
              >
            ...
            <Col span={12}>
              <Form.Item
                ...
                rules={[{ required: false, message: '请选择校准频次!' }]}
              >
            ...
            <Col span={12}>
              <Form.Item
                label="实际起始日期"
                name="BeginTime"
                rules={[{ required: true, message: '请选择实际起始日期!' }]}
              >
