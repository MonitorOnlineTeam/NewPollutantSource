...
export default function createThem(AMap, data, key, map, gif) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    // var aqi = data[i]['AQI'];
    var aqi = data[i][key + '_level'] * 1;
    var x = data[i].Longitude;
    var y = data[i].Latitude;
    let color = data[i][key + '_color'];
    colors.push(color);
    values.push(aqi);
    lngs.push(x);
    lats.push(y);
  ...
  var drawKriging = function () {
    ...
    if (values.length >= 3) {
      ...
      var grid = kriging.grid(polygon, variogram, (ylim[1] - ylim[0]) / 200);
      // 创建新图层
