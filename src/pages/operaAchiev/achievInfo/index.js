...
import { PlusOutlined, UpOutlined, DownOutlined, EditOutlined, ExportOutlined, ProfileOutlined, CreditCardFilled, ProfileFilled, DatabaseFilled } from '@ant-design/icons';
...
const dvaDispatch = (dispatch) => {
  return {
    ...
    updatePersonalPerformanceRateInfo: (payload, callback) => { //工作总量绩效 修改
      dispatch({
        type: `${namespace}/updatePersonalPerformanceRateInfo`,
        payload: payload,
        callback: callback,
      })
    },
...
const Index = (props) => {
  ...
  const edit = (record) => {
    ...
    setEditTitle(`${record.UserName} - 修改`)
  }
    ...
    >
      ...
      <Row>
        <Form.Item label='项目号' name='ProjectNum' className='form_label_width_69'>
          <Input placeholder='请输入' allowClear={true}  style={{ width: 228 }}/>
        </Form.Item>
        <Form.Item label='企业名称' name='EntName'>
          <Input placeholder='请输入' allowClear={true} />
        </Form.Item>
    ...
    <div className={styles.achievQuerySty}>
      <BreadcrumbWrapper>
        ...
        <Modal
          ...
          destroyOnClose
          confirmLoading={props.updatePersonaLoading}
          onOk={editOk}
          centered
          wrapClassName={styles.updatePersonaSty}
          width={600}
        >
          <Form
            name="advanced_search_edit"
            form={editForm}
          >
            <Form.Item label='设备类别系数' name='PointCoefficient' rules={[{ required: true, message: '请输入设备类别' }]}>
