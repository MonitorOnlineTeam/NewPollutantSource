 ...
 * @Last Modified time: 2023-03-22 09:08:58 09:08:58
 */
...
export default Model.extend({
  ...
  effects: {
    ...
    *getPointChartData({ payload }, { call, put, update, select }) {
      const { beginTime, endTime, pollutantCodes, enterpriseCodes } = yield select(
    ...
    *getPointDaysData({ payload }, { call, put, update, select }) {
      const { clickDate, pollutantCodes, queryDGIMNs, enterpriseCodes } = yield select(
        state => state.emissions,
      );
      let body = {
        EnterpriseCodes: enterpriseCodes,
