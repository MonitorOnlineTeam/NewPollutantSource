...
import * as services from '../services/cruxParSupervisionAnalysis';
...
export default Model.extend({
  ...
  effects: {
    *getKeyParameterAnalyseList({ payload, callback }, { call, put, update }) { //关键参数核查分析 列表
      ...
      const result = yield call(services.GetKeyParameterStatisticsList, payload);  
      if (result.IsSuccess) {
    ...
    *exportKeyParameterAnalyseList({ payload, callback }, { call, put, update }) { // 关键参数核查分析 导出
      ...
      const result = yield call(services.ExportKeyParameterStatisticsList, payload);  
      if (result.IsSuccess) {
