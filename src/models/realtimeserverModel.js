...
export default Model.extend({
    ...
    reducers: {
        ...
        updateFlows(state, { payload }) {
            ...
            if (data.length && data[0].DGIMN === state.DGIMN) {
                // let flows = data.filter(item => item.pollutantName.indexOf('流速') > -1 && item.pollutantCode.indexOf('_') > -1);
                // 排除掉流速
                if (state.paramsInfo) {
                    let _paramsInfo = [...state.paramsInfo];
