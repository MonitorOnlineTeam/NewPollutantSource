...
import {
    ...
    IsTrueSerialNumber,
    DeleteCamera,
} from '@/components/VideoView/ysy/services';
...
export default Model.extend({
    ...
    effects: {
        ...
        *ysyvideourl({ payload }, { call, update, select }) {
            const { ysyvideoListParameters } = yield select(state => state.ysyvideo);
            const body = {
        ...
        *queryhistorydatalisthis({ payload }, { call, update, select }) {
            ...
            const { hisrealdataList } = yield select(state => state.ysyvideo);
            const realdata = [];
