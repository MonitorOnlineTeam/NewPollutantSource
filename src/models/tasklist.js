...
export default Model.extend({
  ...
  effects: {
    ...
    *loadAllTaskAuditRecordList({ payload }, { call, update, select }) {
     ...
     * @param {操作} 操作项
     */
    *GetEnterListByName({ payload }, { call, update }) {
      const result = yield call(GetEnterListByName, {
        ...payload,
      });

      if (result.requstresult == EnumRequstResult.Success) {
        yield update({
          EnterList: result.data,
        });
      } else {
        yield update({
          EnterList: [],
     ...
     * @param {操作} 操作项
     */
    *GetPointInfoByEntNoPage({ payload }, { call, update }) {
      const result = yield call(GetPointInfoByEntNoPage, {
        ...payload,
      });
      if (result.requstresult == EnumRequstResult.Success) {
        yield update({
          OneEnterPointList: result.data,
        });
      } else {
        yield update({
          OneEnterPointList: [],
