...
import {
  ...
  GetChildCems,
  AddAnalyzer,
  GetAnalyzerListByDGIMN,
  factoryTest,
  getEnterpriseCorporationCode,
  UpdatePointDGIMN,
  GetMonitorPointVerificationItem,
  GetMonitorPointVerificationList,
  AddMonitorPointVerificationItem,
  AddPointParamInfo,
  GetParamInfoList,
  GetParamCodeList,
  GetPointEquipmentInfo,
  AddOrUpdateEquipmentInfo,
  GetPointEquipmentParameters,
  ...
  GetMonitoringTypeList2,
  GetMonitoringCategoryType,
  GetPBList,
  PointSort,
  AddOrEditPointCoefficient,
  GetPointCoefficientByDGIMN,
  GetPointElectronicFenceInfo,
  AddOrUpdatePointElectronicFenceInfo,
  UpdatePointOprationStatus,
  GetOprationStatusList,
  GetCraftByIndustry,
  AddOrUptCraftByPoint,
  GetCraftByPoint,
...
export default Model.extend({
  ...
  effects: {
    ...
    *getMonitorPointVerificationList({ callback, payload }, { call, put, update, select }) {
      ...
      if (result.IsSuccess) {
        if (result.Datas) {
          if (result.Datas.Verificatiolist) {
            //核查项
    ...
    *getMonitoringTypeList({ payload, callback }, { call, put, update }) {
      //获取监测类别
    ...
    *getPollutantById2({ payload, callback }, { call, put, update }) {
      //获取监测参数
      if (payload.id) {
    ...
    *getEquipmentInfoList({ payload, callback }, { call, put, update }) {
      //列表 设备信息
    ...
    *getPointCoefficientByDGIMN({ payload, callback }, { call, put, update }) {
      //获取监测点系数
    ...
    *addOrEditPointCoefficient({ payload, callback }, { call, put, update }) {
      //添加或修改监测点系数
      const result = yield call(AddOrEditPointCoefficient, {
        ...payload,
    ...
    *getOprationStatusList({ payload, callback }, { call, put, update }) {
      ...
      if (result.IsSuccess) {
        yield update({
          oprationStatusList: result?.Datas?.dataList ? result.Datas.dataList : [],
          oprationStatusListTotal: result.Total,
        });
        callback && callback(result.Datas);
      } else {
        result.Message && message.error(result.Message);
