...
export default Model.extend({
  ...
  effects: {
    ...
    *DelEnum({ payload }, { call, put, update, select }) {
    ...
    // 获取已保存的按钮
    *GetSelectedButton({ payload, callback }, { call, select, update }) {
      const result = get(API.AutoFormApi.GetSelectedButton, payload, null);
    ...
    // 保存已选择的按钮
    *SaveSelectedButton({ payload, callback }, { call, select, update }) {
      const result = post(API.AutoFormApi.SaveSelectedButton, payload, null);
      result.then(res => {
        if (res.IsSuccess) {
          callback(res.Datas);
        } else {
