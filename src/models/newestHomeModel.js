...
export default Model.extend({
  ...
  state: {
    ...
    exceptionSignTaskRateList: {
      insidePlanRate: "0.00", insidePlanTaskCount: 0, insidePlanTaskExceptionCount: 0,
  ...
  effects: {
    ...
    *GetOperationPlanTaskListByDay({ payload, callback }, { call, put, update }) { //工单执行情况 详情
      const result = yield call(requestPost, API.VisualKanbanApi.GetOperationPlanTaskListByDay, payload);
      if (result.IsSuccess) {
        callback && callback(result.Datas)
      }
    },
    ...
    *GetPlanOperationTaskCompleteRateByDay({ payload, callback }, { call, put, update }) {  //近30日运维情况 固定到天
      const result = yield call(requestPost, API.VisualKanbanApi.GetPlanOperationTaskCompleteRateByDay, payload);
      if (result.IsSuccess) {
        yield update({ planOperaList: result.Datas });
      }
    },
