...
class NavigationTree extends Component {
  ...
  componentDidMount() {
    ...
    dispatch({
      ...
      payload: {
        ...
        // PollutantTypes:
        // this.props.polShow && this.props.type == 'ent'
        //   ? '1,2'
        //   : this.props.polShow && this.props.type == 'air'
        //   ? '5'
      ...
      if (where) {
        ...
        if (selKeys || this.props.selKeys) {
          ...
          if (overAll || this.props.overAll) {
            ...
            this.setLocalStorage([nowExpandKey], nowKey, {
              entName: node.EntName,
              pointName: node.title,
            });
          }
  ...
  handleChange = value => {
    ...
    if (value == '') {
      value = this.props.type == 'ent' ? '1,2' : value;
      // this.props.ConfigInfo.SystemPollutantType
    }
    this.setState({
  ...
  onChangeSearch = (e, data) => {
    ...
    const entList = entAndPoint.filter(
      item => item.title.toUpperCase().indexOf(msg) > -1 && item.IsEnt == 1,
    );
    const filterList = this.state.panelDataListAys.filter(
      item =>
        item.pointName.toUpperCase().indexOf(msg) > -1 ||
        item.entName.toUpperCase().indexOf(msg) > -1,
    );
  ...
  returnData = data => {
    ...
    if (rtnList.length == 0) {
      ...
      this.setLocalStorage(this.state.expandedKeys, this.state.selectedKeys, {
        entName: rtnList[0].entName,
        pointName: rtnList[0].pointName,
      });
    }
  };
  ...
  getEntIcon = type => {
    switch (type) {
      ...
      case '3':
        return (
          <a>
            <CustomIcon type="icon-heliu1" style={{ fontSize: 16 }} />
          </a>
        );
      case '4':
        return (
  ...
  getPollutantIcon = (type, size) => {
    switch (type) {
      case '1':
        return (
          <a>
            <WaterIcon style={{ fontSize: size }} />
          </a>
        );
      case '2':
        return (
          <a>
            <GasIcon style={{ fontSize: size }} />
          </a>
        );
      case '6':
        return (
          <a>
            <CustomIcon
              ...
              style={{ fontSize: size, color: '#3c99d8' }}
            />
          </a>
        );
      case '9':
        return (
          <a>
            <CustomIcon type="icon-echoujiance" style={{ fontSize: size, color: '#3c99d8' }} />
          </a>
        );
      case '10':
        return (
          <a>
            <VocIcon style={{ fontSize: size }} />
          </a>
        );
      case '12':
        return (
          <a>
            <CustomIcon type="icon-yangchen1" style={{ fontSize: size }} />
          </a>
        );
      case '5':
        return (
          <a>
            <CustomIcon type="icon-fangwu" style={{ fontSize: size }} />
          </a>
        );
      case '37':
        return (
          <a>
            <CustomIcon type="icon-dian2" style={{ fontSize: size }} />
          </a>
        );
    }
  };
  shouldComponentUpdate(nextProps, nextState) {
    ...
    // }
    if (
      nextProps.noticeList.length &&
      this.props.noticeList.length === nextProps.noticeList.length &&
            ...
            <TreeNode
              ...
              title={
                <div style={{ width: '254px', position: 'relative' }}>
                  <div className={styles.titleStyle} title={item.title} style={{ width: 190 }}>
      ...
      <div>
        ...
        <div
          ...
          style={{
            ...
            transition:
              'all  0.3s cubic-bezier(0.7, 0.3, 0.1, 1) 0s, box-shadow 0.3s cubic-bezier(0.7, 0.3, 0.1, 1) 0s',
            display: 'flex',
            width: '18px',
            height: '48px',
            size: '16px',
