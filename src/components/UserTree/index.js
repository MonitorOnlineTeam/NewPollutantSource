...
import { ManIcon, WomanIcon } from '@/utils/icon';
...
class UserTree extends Component {
  ...
  setRowClassName = record =>
    record.UserID === this.state.selectedKeys[0] ? global.clickRowStyl : '';
        ...
        render: (text, record) => {
            ...
            <div style={{ fontSize: '14px', fontWeight: 'bolder' }}>
              ...
              {record.Phone && (
                <div style={{ float: 'right', marginRight: '10px' }}>
                  <MobileTwoTone style={{ marginRight: '5px' }} />
                  {record.Phone}
                </div>
              )}
            </div>,
          ...
          record.UserGroupName &&
            record.UserGroupName.split(',').map((m, index) => {
              if (index < 2) {
                itemlist.push(
                  <Tag title={m} color="geekblue" styles={{ fontSize: 5 }}>
                    {m.length > 5 ? `${m.substring(0, 5)}...` : m}
                  </Tag>,
                );
              }
              if (index === 2) {
                itemlist.push(
                  <Tag title={record.UserGroupName} color="geekblue" styles={{ fontSize: 5 }}>
                    ...
                  </Tag>,
                );
              }
          ...
          record.RolesName &&
            record.RolesName.split(',').map((m, index) => {
              if (index < 2) {
                itemlist.push(
                  <Tag title={m} color="orange" styles={{ fontSize: 5 }}>
                    {m.length > 5 ? `${m.substring(0, 5)}...` : m}
                  </Tag>,
                );
              }
              if (index === 2) {
                itemlist.push(
                  <Tag title={record.RolesName} color="orange" styles={{ fontSize: 5 }}>
                    ...
                  </Tag>,
                );
              }
