...
class Index extends Component {
  ...
  render() {
    const {
      alarmPushData,
      showAlarmState,
      alarmPushParam: { pageIndex, pageSize, total },
      loadingGetData,
      loadingGetAlarmState,
      loadingInsertData,
    } = this.props;
    ...
    return (
      <div>
        ...
        >
          <Row gutter={16}>
            {loadingGetData || loadingGetAlarmState ? (
              <Spin
                style={{
                  width: '100%',
                  height: 'calc(100vh/2)',
                ...
                size="large"
              />
            ) : alarmPushData != null && alarmPushData.length > 0 ? (
              alarmPushData.map((item, index) => (
                <Col span={6}>
                  <Card
                    title={<span style={{ fontSize: 14 }}>{item.TargetName}</span>}
                        ...
                        <Checkbox
                          ...
                          checked={
                            currentData.filter(
                              m =>
                                m.DGIMN === item.DGIMN &&
                                (m.AlarmTypes && m.AlarmTypes.indexOf('1') > -1),
                            ).length > 0
                          }
                        ...
                        <Checkbox
                          ...
                          checked={
                            currentData.filter(
                              m =>
                                m.DGIMN === item.DGIMN &&
                                (m.AlarmTypes && m.AlarmTypes.indexOf('2') > -1),
                            ).length > 0
                          }
                          onChange={this.onChangeDGINM}
                        >
                          ...
                          <Checkbox
                            ...
                            checked={
                              currentData.filter(
                                m =>
                                  m.DGIMN === item.DGIMN &&
                                  (m.AlarmTypes && m.AlarmTypes.indexOf('5') > -1),
                              ).length > 0
                            }
                            onChange={this.onChangeDGINM}
                          >
        ...
        <div style={{ textAlign: 'right' }}>
          <Button
            type="default"
            style={{ marginRight: 5 }}
            onClick={this.props.cancelModal && this.props.cancelModal}
          >
            取消
          </Button>
          <Button type="primary" onClick={this.saveData} loading={loadingInsertData}>
            保存
          </Button>
        </div>
      </div>
