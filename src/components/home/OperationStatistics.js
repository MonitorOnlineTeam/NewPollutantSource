    ...
    return (
      <>
        ...
        <div className={styles.content}>
          <p className={styles.operationsNumber}>
            {currentMonth}月共<span>{taskCountData.TaskSum}</span>次运维任务
          </p>
          ...
          <div className={styles.progressInfo}>
            <p>
              已完成<span style={{ color: '#61c302' }}>{taskCountData.CompletedTaskSum}</span>次
            </p>
            <p>
              未完成<span style={{ color: '#f30201' }}>{taskCountData.NoCompletedTaskSum}</span>次
            </p>
          </div>
          <div className={styles.statisticContent}>
            <p>
              {currentMonth}月质控智能预警<span>{operationsWarningData.ThisMonthEP}</span>次
            </p>
            <div className={styles.statisticInfo}>
              {operationsWarningData.ThisMonthTB > 0 ? (
                <Statistic
                  title={<span style={{ color: '#accdec' }}>同比</span>}
                  valueStyle={{ color: '#5bf287', fontSize: 18 }}
                  value={operationsWarningData.ThisMonthTB}
                  prefix={<CaretUpOutlined />}
                />
              ) : (
                <Statistic
                  title={<span style={{ color: '#accdec' }}>同比</span>}
                  valueStyle={{ color: '#FF4E4E', fontSize: 18 }}
                  value={operationsWarningData.ThisMonthTB}
              ...
              {operationsWarningData.ThisMonthHB > 0 ? (
                <Statistic
                  title={<span style={{ color: '#accdec' }}>环比</span>}
                  valueStyle={{ color: '#5bf287', fontSize: 18 }}
                  value={operationsWarningData.ThisMonthHB}
                  prefix={<CaretUpOutlined />}
                />
              ) : (
                <Statistic
          ...
          <div className={styles.abnormalAlarmContent}>
            ...
            <div className={styles.content}>
              ...
              <div className={styles.chartDescription}>
                <div className={styles.twoHours}>
                  两小时内响应({alarmAnalysis.LessThan2Hour})次
                  <br />
                  {alarmAnalysis.LessThan2Hourlink > 0
                    ? `环比上升${alarmAnalysis.LessThan2Hourlink}%`
                    : `环比下降${Math.abs(alarmAnalysis.LessThan2Hourlink)}%`}
                </div>
                <div className={styles.eightHours}>
                  超八小时响应({alarmAnalysis.GreaterThan8Hour})次
                  <br />
                  {alarmAnalysis.GreaterThan8Hourlink > 0
                    ? `环比上升${alarmAnalysis.GreaterThan8Hourlink}%`
                    : `环比下降${Math.abs(alarmAnalysis.GreaterThan8Hourlink)}%`}
                </div>
