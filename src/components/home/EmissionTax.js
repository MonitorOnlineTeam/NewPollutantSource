    ...
    return (
      <>
        ...
        <div className={styles.content}>
          <p className={styles.yjTax}>
            <span
              style={{ textAlign: 'left', fontSize: '16px', fontWeight: 600, color: '#d5d9e2' }}
            >
              {moment(taxInfo.Date).format('MM') * 1}月应交环保税：
            </span>
            <Statistic
              valueStyle={{ color: '#fff', textAlign: 'center', fontSize: 22 }}
              value={taxInfo.LastQuarter}
              prefix="￥"
            />
          </p>
          <Row gutter={16}>
            <Col span={12}>
              <Statistic
                title={<span style={{ color: '#d5d9e2' }}>环保税：</span>}
                valueStyle={{ color: '#fff', fontSize: 18 }}
                value={taxInfo.EffluentFeeValue}
                prefix="￥"
              />
            </Col>
            <Col span={12} style={{ textAlign: 'right' }}>
              <Statistic
                title={<span style={{ color: '#d5d9e2' }}>超低排放奖励：</span>}
                valueStyle={{ color: '#fff', fontSize: 18 }}
                value={taxInfo.UltralowEmissionIncentives}
                prefix="￥"
              />
            </Col>
          </Row>
          ...
          <div className={styles.quarterTax}>
            ...
            {taxInfo.ThisQuarter > taxInfo.LastQuarter ? (
              <Statistic
                ...
                prefix={
                  <>
                    <CaretUpOutlined style={{ color: '#FF4E4E', fontSize: '18px' }} /> ￥{' '}
                  </>
                }
              />
            ) : (
              <Statistic
                ...
                prefix={
                  <>
                    <CaretDownOutlined style={{ color: 'rgb(91, 242, 135)', fontSize: '18px' }} />{' '}
                    ￥{' '}
                  </>
                }
              />
