...
const Index = props => {
  ...
  function findParentNodeByKey(nodes, childNodeKey) {
    //通过子节点的key 查找父节点  并返回其key  以数组形式返回
  ...
  const dealCheckboxSeleted = ({ node, onItemSelect, onItemSelectAll }) => {
    ...
    if (children?.length > 0) {
      ...
      if (!checked) {
        // 查找该元素的父元素
        let parentKeys =
          (halfCheckedKeys?.[0] && [halfCheckedKeys?.[0]]) ||
          findParentNodeByKey(treeData, key) ||
          [];
  ...
  const TreeTransfer = ({ dataSource, targetKeys, ...restProps }) => {
    ...
    let leftTreeData = singleLayer
      ? leftData
      : leftData?.length
      ? leftData.filter(item => item.children[0])
      : [];
    return (
      <Transfer
        {...restProps}
  ...
  const onChange = (keys, direction, moveKeys) => {
    ...
    props.targetKeysChange
      ? props.targetKeysChange(moveKeys, changeArrType, () => {
