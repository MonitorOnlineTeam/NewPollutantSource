...
function kriging(id, pixelsize) {
    ...
    this.map = function (w, h, center, zoom) {
        ...
        this.canvas.height = h //window.innerHeight - this.canvas.offsetTop - 20;
    ...
    function gradientColor(startColor, endColor, step) {
        startRGB = this.colorRgb(startColor);//ת��Ϊrgb����ģʽ
        startR = startRGB[0];
        startG = startRGB[1];
        startB = startRGB[2];
        ...
        console.log("��ɫ����" + colorArr.toString())
        return colorArr;
    }
            ...
            for (j = xbox[0]; j <= xbox[1]; j += this.xpixel) {
                for (k = ybox[0]; k <= ybox[1]; k += this.ypixel) {
                    if (pip(this.polygons[nearest][0], this.polygons[nearest][1], j, k)) {
                        ...
                        //this.pixel(j, k, this.colorspectrum.heatcolors[color])
